...
const ALLOWED = "mr3324333770@gmail.com";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "setup") {
      ...
      const otpauth = authenticator.keyuri(user.email!, "Jalwa Pro Payment Accounts", secret);
      await admin.from("admin_payment_2fa").upsert({ user_id: user.id, email: user.email!.toLowerCase(), secret, enabled: false, updated_at: new Date().toISOString() });
    ...
    if (action === "enable") {
      ...
      await admin.from("admin_payment_2fa").update({ enabled: true, updated_at: new Date().toISOString() }).eq("user_id", user.id);
    ...
    console.error(error);
    return json({ error: error instanceof Error ? error.message : "2FA error" }, 500);
  }
});

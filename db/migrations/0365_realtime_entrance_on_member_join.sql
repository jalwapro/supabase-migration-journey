-- 0365: Every actual room membership join creates a durable entrance event.
-- Equipped entrance => media/animation payload.
-- No equipped entrance => identity-only entrance bar; joining must never be silent.
...
BEGIN
  ...
  SELECT e.* INTO _eff
   ...
   WHERE u.user_id = _user_id AND u.is_equipped = true AND e.is_active = true
     AND (u.expires_at IS NULL OR u.expires_at > now())
   LIMIT 1;
  _has_effect := FOUND;

  SELECT username, avatar_url, vip_level, country
    INTO _prof FROM public.profiles WHERE id = _user_id;
  ...
  VALUES
    (_room_id,_user_id,
     ...
     CASE WHEN _has_effect THEN COALESCE(_eff.duration_ms,2400) ELSE 2400 END,
     _prof.username,_prof.avatar_url,COALESCE(_prof.vip_level,0),_prof.country,
     CASE WHEN _has_effect THEN COALESCE(_eff.render_config,'{}'::jsonb) ELSE '{}'::jsonb END)
  RETURNING * INTO _row;
...
FOR EACH ROW EXECUTE FUNCTION public.trg_room_member_entrance();

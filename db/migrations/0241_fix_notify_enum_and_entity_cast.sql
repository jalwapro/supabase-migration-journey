-- Cast notification enum + entity_id types explicitly.
...
begin
  if new.status is distinct from old.status
    ...
    values (
      new.user_id,
      (case when new.status = 'approved' then 'withdrawal_approved' else 'withdrawal_rejected' end)::public.notification_kind,

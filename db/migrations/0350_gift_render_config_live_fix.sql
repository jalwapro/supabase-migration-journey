-- 0350: Production repair for Gift Studio render-config persistence.
-- Idempotent because older source migrations may exist while production drifted.
begin;
...
BEGIN
  IF NEW.sender_username IS NULL AND NEW.sender_id IS NOT NULL THEN
    SELECT username, avatar INTO NEW.sender_username, NEW.sender_avatar
      FROM public.profiles WHERE id = NEW.sender_id;
  END IF;
  IF NEW.receiver_username IS NULL AND NEW.receiver_id IS NOT NULL THEN
    SELECT username, avatar INTO NEW.receiver_username, NEW.receiver_avatar
  ...
  IF NEW.gift_id IS NOT NULL THEN
    SELECT name, emoji, icon, animation, clip_path, clip_type, image_url,
      ...
      INTO NEW.gift_name, NEW.gift_emoji, NEW.gift_icon, NEW.gift_animation,
           ...
           NEW.gift_loop, NEW.gift_audio_volume, NEW.gift_render_config

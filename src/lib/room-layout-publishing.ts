...
export async function publishRoomLayout(layoutId: string, layout: LayoutJSON, type: RoomType) {
  ...
  const { error: versionError } = await supabase.from('room_layout_versions').insert({
    ...
    version: nextVersion,
    layout_json: layout,
    change_description: 'Published from Room Layout Studio',
  });
  if (versionError) throw versionError;

  if (typeof window !== 'undefined') {
    window.dispatchEvent(new CustomEvent('jalwa:room-layout-published', {
      detail: { layoutId, roomType: type, version: nextVersion },
    }));
  }

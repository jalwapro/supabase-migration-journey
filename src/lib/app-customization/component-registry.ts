...
export interface StudioComponentDefinition {
  type: ComponentType;
  label: string;
  category: "layout" | "content" | "controls" | "navigation" | "jalwa" | "overlay" | "media";
  editable: EditablePropertyGroup[];
...
const definitions: StudioComponentDefinition[] = [
  ...
  ["video-container", "Video Container", "media", false], ["audio-container", "Audio Container", "media", false],
  ["popup", "Popup", "overlay", true], ["modal", "Modal", "overlay", true], ["dialog", "Dialog", "overlay", true], ["bottom-sheet", "Bottom Sheet", "overlay", true], ["drawer", "Drawer", "overlay", true], ["toast", "Toast", "overlay", true], ["tooltip", "Tooltip", "overlay", true], ["dropdown", "Dropdown", "overlay", true], ["menu", "Menu", "overlay", true], ["overlay", "Overlay", "overlay", true]
].map(([type, label, category, children]) => ({
  type: type as ComponentType,
  label: String(label),
  category: category as StudioComponentDefinition["category"],
  editable: type === "text" || type === "heading" || type === "paragraph" ? [...content, "typography"] : type === "image" || type === "icon" || type === "avatar" ? [...content] : [...common],
  supportsChildren: children === true || children === "children",
  supportsBinding: ["live-room-card", "voice-room-card", "video-room-card", "pk-battle-card", "user-profile-card", "gift-card", "gift-grid", "coin-balance", "diamond-balance", "recharge-packages", "ranking-list", "leaderboard", "room-seat-layout", "list", "list-item"].includes(type as ComponentType),
  supportsInteraction: ["button", "icon-button", "follow-button", "live-button", "create-room-button", "pk-battle-button", "navigation", "bottom-navigation", "tabs", "popup", "modal", "dialog", "bottom-sheet", "drawer", "dropdown", "menu"].includes(type as ComponentType)
}));

export const STUDIO_COMPONENT_REGISTRY: Record<ComponentType, StudioComponentDefinition> = Object.fromEntries(definitions.map((definition) => [definition.type, definition])) as Record<ComponentType, StudioComponentDefinition>;

export function getStudioComponentDefinition(type: ComponentType) {
  return STUDIO_COMPONENT_REGISTRY[type];
}

export function getStudioComponentsByCategory(category: StudioComponentDefinition["category"]) {

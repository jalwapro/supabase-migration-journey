...
function RoomsPage() {
  ...
  const rooms = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data ?? [])
        .filter((r: { room_type: string }) => r.room_type === "voice")
        .map((r: {
  ...
  return (
    <>
      <AppShell title="Voice Rooms" subtitle="Browse all live voice rooms">
        <div className="px-4 pt-3">
          {rooms.isLoading ? (
                ...
                >
                  ...
                  <div className="absolute inset-x-2 bottom-2">
                    <div className="flex items-center gap-1 text-[10px] text-white/80">
                      <Mic className="h-3 w-3" /> {r.host?.username ?? "host"}
                    </div>
            ...
            <div className="glass mt-4 rounded-2xl p-8 text-center">
              ...
              <p className="mt-3 text-sm font-semibold">No live voice rooms right now</p>
              <Link
                to="/create-room"

...
const PipelineBridge = ({ bridgeRef }: { bridgeRef: React.MutableRefObject<any> }) => {
  const pipeline = useCamPipeline();
  React.useEffect(() => {
    bridgeRef.current = {
      processStream: pipeline.processStream,
      releaseProcessor: pipeline.releaseProcessor,
    };
    return () => {
      bridgeRef.current = null;
    };
  }, [pipeline.processStream, pipeline.releaseProcessor, bridgeRef]);
  return null;
};
...
function RoomPage() {
  ...
  const roomStyle: React.CSSProperties = {
    background: "linear-gradient(180deg, #0f041e 0%, #050505 100%)",
  ...
  const mappedRoomState: RoomState = (() => {
    ...
    const hostParticipant: RoomParticipant = {
      id: r.host_id,
      username: r.host?.username ?? "Host",
      avatar: r.host?.avatar ?? null,
      level: r.host?.vip_level ?? 0,
      is_vip: (r.host?.vip_level ?? 0) > 0,
      is_muted: hostMember ? hostMember.is_muted : agora.muted,
      is_speaking: hostMember ? agora.speakingUids.has(uidFromUuid(hostMember.user_id)) : false,
      has_video: hostMember ? hostMember.is_video : false,
    ...
    const roomSeats: RoomSeat[] = Array.from({ length: Math.max(0, r.seat_count - 1) }, (_, i) => {
      ...
      return {
        ...
        user: m ? {
          ...
          avatar: m.user?.avatar ?? null,
          level: m.user?.vip_level ?? 0,
          is_vip: (m.user?.vip_level ?? 0) > 0,
          is_muted: m.is_muted,
          is_speaking: agora.speakingUids.has(uidFromUuid(m.user_id)),
          has_video: m.is_video,
          gift_score: giftPoints[m.user_id] ?? 0
        } : null
      };
    });
    ...
    return {
      ...
      viewer_count: Math.max(r.viewer_count, members.length),
      seats: roomSeats,
      pk: null
    };
  })();
...
<HostRoomMoreSheet open={hostMoreOpen} onClose={() => setHostMoreOpen(false)} seatCount={r.seat_count} maxSeats={20} onSeats={async (next) => { if (!isHost) return; const occupied=members.filter((m)=>m.seat_index!=null).length; if(next<occupied){toast.error(`Cannot reduce seats below ${occupied} occupied seats`);return;} const {error}=await supabase.rpc("update_voice_room_seat_count",{_room_id:roomId,_seat_count:next}); if(error){toast.error(error.message);return;} setHostMoreOpen(false); await room.refetch(); }} onMusic={() => {setHostMoreOpen(false);setMusicOpen(true)}} onSettings={() => {setHostMoreOpen(false);setVideoSettingsOpen(true)}} onInvite={() => {setHostMoreOpen(false);setInviteOpen(true)}} onRanking={() => {setHostMoreOpen(false);void navigate({to:"/rank"})}} />

<JalwaPrivateChat open={privateChatOpen} onClose={() => setPrivateChatOpen(false)} />

{chatComposerOpen && (
<div
className="fixed inset-0 z-[2147483000] flex flex-col justify-end bg-black/60 backdrop-blur-sm"
onClick={() => setChatComposerOpen(false)}
>
<div
className="flex max-h-[75vh] w-full flex-col overflow-hidden rounded-t-3xl border-t border-fuchsia-400/30 bg-[#0d0616]"
...
{/* Message list */}
<div className="min-h-0 flex-1 space-y-1.5 overflow-y-auto px-3 py-3 scrollbar-hide">
...
{/* Composer */}
<div
className="shrink-0 border-t border-white/10 bg-gradient-to-t from-black via-black/95 to-transparent px-4 pb-4 pt-3 backdrop-blur-md"
style={{ paddingBottom: `calc(${keyboardOffset}px + 12px)` }}
>
...
className="max-h-32 w-full resize-none rounded-2xl border border-white/10 bg-white/5 p-3 text-sm text-white focus:outline-none focus:ring-1 focus:ring-fuchsia-500/50"
onKeyDown={(e) => {
...
onClick={() => void send()}
disabled={!text.trim()}
className="flex h-11 w-11 items-center justify-center rounded-full bg-gradient-to-r from-fuchsia-500 to-purple-600 text-white disabled:opacity-50"
>
...
const ChatLine = React.memo(function ChatLine({
  ...
  const mentionsMe =
    !!myName && (m.mentions ?? []).some((n) => typeof n === 'string' && n.toLowerCase() === myName.toLowerCase());
  return (

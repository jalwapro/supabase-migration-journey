...
import { Monitor, Smartphone, Tablet, ExternalLink, RefreshCw, Save, Upload, Eye } from 'lucide-react';
...
const pages = [
  ['home', '/', 'Home'], ['discover', '/discover', 'Discover'], ['live', '/live', 'Live'], ['rooms', '/rooms', 'Rooms'],
  ['profile', '/profile', 'Profile'], ['wallet', '/wallet', 'Wallet'], ['recharge', '/recharge', 'Recharge'], ['gifts', '/gifts', 'Gifts'],
  ['ranking', '/ranking', 'Ranking'], ['chat', '/chat', 'Chat'], ['messages', '/messages', 'Messages'], ['notifications', '/notifications', 'Notifications'],
  ['settings', '/settings', 'Settings'], ['login', '/login', 'Login'], ['register', '/register', 'Register'], ['splash', '/splash', 'Splash'],
];
...
function ExistingAppPreview() {
  ...
  const src = useMemo(() => `${window.location.origin}${selectedPage[1]}?adminPreview=1`, [selectedPage]);
  ...
  const save = async (publish: boolean) => {
    ...
    const config = { schemaVersion: 2, page, elements: rules };
    ...
    if (publish) {
      ...
      setStatus(`Published ${selectedPage[2]} v${nextVersion}. Open the user app to verify.`);
  ...
  return (
    <div className="-m-4 min-h-[calc(100vh-64px)] bg-muted/20 md:-m-6 lg:-m-8">
      <div className="sticky top-0 z-30 border-b bg-background/95 px-4 py-3 backdrop-blur md:px-6">
        <AdminPageHeader title="App Customization — Visual Editor" subtitle="Select the real app elements, edit them here, save a draft, then publish the configuration." right={<div className="flex gap-2"><button className="rounded-lg border p-2" onClick={() => setReloadKey((v) => v + 1)} title="Reload"><RefreshCw className="h-4 w-4" /></button><a className="rounded-lg border px-3 py-2 text-sm" href={src} target="_blank" rel="noreferrer"><ExternalLink className="mr-1 inline h-4 w-4" />Open App</a></div>} />
        <div className="mt-3 flex flex-wrap items-center gap-2">
          <select value={page} onChange={(e) => { setPage(e.target.value); setRules([]); setSelected(null); }} className="rounded-lg border bg-background px-3 py-2 text-sm">{pages.map(([key, , label]) => <option key={key} value={key}>{label}</option>)}</select>
      ...
      <main className="grid min-h-[calc(100vh-160px)] grid-cols-[minmax(0,1fr)_300px] overflow-hidden bg-zinc-950">
        <section className="overflow-auto p-4 md:p-8">
          <div className="mb-2 flex items-center justify-between text-[10px] text-zinc-400"><span><Eye className="mr-1 inline h-3 w-3" />LIVE EXISTING APP — SELECT MODE</span><span>{selectedPage[2]} • {width}px</span></div>
        ...
        <aside className="overflow-y-auto border-l border-white/10 bg-background p-4">
          ...
          {!selected ? <p className="mt-3 text-sm text-muted-foreground">Click any visible element in the app preview to select it.</p> : <>
          ...
          <div className="mt-6 rounded-xl border border-primary/20 bg-primary/5 p-3 text-xs text-muted-foreground">Changes are stored in the customization database. Publishing creates a new page version; the existing business logic remains unchanged.</div>
        </aside>
      </main>
    </div>
  );

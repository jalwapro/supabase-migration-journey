...
import { Plus, Trash2, Image as ImageIcon, Pencil, X, Save } from "lucide-react";
...
type Draft = { title: string; image_url: string; link_url: string; sort_order: number };
const empty: Draft = { title: "", image_url: "", link_url: "", sort_order: 99 };
...
function BannersAdmin() {
  ...
  const update = useMutation({
    mutationFn: async () => {
      ...
      if (!editDraft.image_url.trim()) throw new Error("Image required");
          ...
          <div key={b.id} className="glass rounded-2xl p-3">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="truncate text-xs text-muted-foreground">{b.link_url ?? "no link"} · #{b.sort_order}</p>
              </div>
              <button
            ...
            {editingId === b.id && (
              <div className="mt-3 rounded-xl border border-primary/30 bg-card/40 p-3">
                <div className="mb-2 flex items-center justify-between">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-primary">Edit banner</p>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <input placeholder="Title" value={editDraft.title} onChange={(e) => setEditDraft({ ...editDraft, title: e.target.value })} className="rounded-lg border border-border bg-input px-2 py-1.5 text-xs outline-none" />
                  <input placeholder="Order" type="number" value={editDraft.sort_order} onChange={(e) => setEditDraft({ ...editDraft, sort_order: Number(e.target.value) })} className="rounded-lg border border-border bg-input px-2 py-1.5 text-xs outline-none" />
                  <div className="col-span-2">
                    <FileUploader bucket="banners" accept="image/*" label="Replace banner image" value={editDraft.image_url} onChange={(url) => setEditDraft({ ...editDraft, image_url: url ?? "" })} />
                  </div>
                  <input placeholder="Link URL (optional)" value={editDraft.link_url} onChange={(e) => setEditDraft({ ...editDraft, link_url: e.target.value })} className="col-span-2 rounded-lg border border-border bg-input px-2 py-1.5 text-xs outline-none" />
                </div>

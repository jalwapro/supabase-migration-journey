...
function LiveRoomsStudioRoute() {
  ...
  const previewUrl = realRoute ? `${realRoute}?studioPreview=1&previewIdentity=neutral&studioState=normal` : null;

  function sendStudioConfig() {
    iframeRef.current?.contentWindow?.postMessage({ type: "jalwa-live-studio-config", config, pageKey: kind, state: "normal" }, "*");
  }
  ...
  useEffect(() => {
    void (async () => {
      const { data, error } = await supabase
        ...
        .select("id,page_key,name,route_pattern")
        .in("page_key", ["voice-room", "video-room", "pk-battle"]);
      if (error) toast.error(error.message);
      else setPages((data ?? []) as PageRow[]);

      // The real room route is backed by live_rooms. The old studio queried
      // `rooms`, which can be empty even though the application has room
      // records, leaving the canvas as a blank black iframe. Prefer an active
      // room for the selected experience, then fall back to the newest room
      // so the editor remains usable even when no one is live right now.
      const preferredType = kind === "video-room" ? "video" : "voice";
      const active = await supabase
        .from("live_rooms")
        .select("id,room_type,status")
        .eq("room_type", preferredType)
        .eq("status", "live")
      ...
      if (!room) {
        const fallback = await supabase
          ...
          .limit(1)
          .maybeSingle();
        room = (fallback.data as RoomRow | null) ?? null;
      }
  ...
  useEffect(() => {
    ...
    void (async () => {
      const { data, error } = await supabase
        .from("app_customization_versions")
        .select("config")
        .eq("page_id", page.id)
        .eq("status", "draft")
  ...
  // Re-select a real room whenever the room type changes.
  useEffect(() => {
    void (async () => {
      const preferredType = kind === "video-room" ? "video" : "voice";
  ...
  return <div className="-m-4 min-h-[calc(100vh-64px)] bg-muted/20 md:-m-6 lg:-m-8">
    ...
    <div className="grid gap-4 p-4 xl:grid-cols-[minmax(0,1fr)_420px]"><section className="rounded-xl border bg-background p-3"><div className="mb-2 flex items-center justify-between"><div><div className="text-xs font-semibold">REAL APPLICATION CANVAS</div><div className="text-[10px] text-muted-foreground">Existing room route with Studio configuration streamed into preview mode.</div></div><span className="text-[10px] text-muted-foreground">{realRoute ?? "No room available"}</span></div>{previewUrl ? <iframe ref={iframeRef} title={`${kind} real application preview`} src={previewUrl} onLoad={sendStudioConfig} className="h-[760px] w-full rounded-xl border bg-black" /> : <div className="grid h-[760px] place-items-center rounded-xl border bg-muted/20 text-sm text-muted-foreground">No existing room record is available for preview. Create a room once, then return here; the editor never creates production room data.</div>}</section>
      <section className="space-y-4 rounded-xl border bg-background p-3"><LiveRoomLayersBridge kind={kind} config={config} onChange={setConfig} iframeRef={iframeRef} /><LiveRoomStudioPanel pageKey={kind} config={config} onChange={setConfig} onSaveDraft={() => void saveDraft()} onPublish={() => void publish()} /></section></div>

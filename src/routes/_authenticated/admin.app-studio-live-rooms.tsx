...
function LiveRoomsStudioRoute() {
  ...
  function sendStudioConfig() { iframeRef.current?.contentWindow?.postMessage({ type: "jalwa-live-studio-config", config, pageKey: kind, state: "normal" }, "*"); }
  ...
  useEffect(() => { void (async () => {
    ...
    if (room?.id) setRoomId(room.id); setLoading(false);
  })(); }, [kind]);
  ...
  return <div className="-m-4 min-h-[calc(100vh-64px)] bg-muted/20 md:-m-6 lg:-m-8">
    ...
    <div className="grid gap-4 p-4 xl:grid-cols-[minmax(0,1fr)_420px]">
      <section className="rounded-xl border bg-background p-3"><div className="mb-2 flex items-center justify-between"><div><div className="text-xs font-semibold">REAL APPLICATION CANVAS</div><div className="text-[10px] text-muted-foreground">Mobile device frame preview — existing room route and live functionality are preserved.</div></div><span className="text-[10px] text-muted-foreground">{realRoute ?? "No room available"}</span></div>

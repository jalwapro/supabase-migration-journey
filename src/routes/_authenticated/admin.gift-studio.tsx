...
import { Loader2, RotateCcw, Save, Undo2, Redo2, Search, Download, Upload, Copy, Crosshair, FlipHorizontal, FlipVertical } from "lucide-react";
...
function GiftStudio() {
  ...
  const { data: gifts = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("gifts")
        .select("id,name,category,clip_path,clip_type,image_url,price,render_config")
  ...
  const { data: presets = [] } = useQuery({
    queryKey: ["admin", "gift-studio", "presets"],
    queryFn: async () => {
  ...
  const filtered = useMemo(() => {
    ...
    return q ? gifts.filter((g) => g.name.toLowerCase().includes(q) || g.category.toLowerCase().includes(q)) : gifts;
  }, [gifts, search]);
  ...
  const save = useMutation({
    mutationFn: async (config: GiftRenderConfig) => {
      if (!selectedId) throw new Error("No gift selected");
      const { error } = await supabase
        .from("gifts")
        .update({ render_config: diffRenderConfig(config) })
        .eq("id", selectedId);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const onPointerMove = (e: React.PointerEvent) => {
    ...
    const ratio = stage ? 1080 / stage.clientWidth : 1;
    if (d.mode === "move") {
      const snap = (v: number) => (Math.abs(v) < 12 ? 0 : Math.round(v));
      setCfgState({ ...d.cfg, positionX: snap(d.cfg.positionX + dx * ratio), positionY: snap(d.cfg.positionY + dy * ratio) });
      setDirty(true);
  ...
  const clipUrl = resolvePlayableGiftUrl(selected?.clip_path ?? null);
  ...
  const exportJson = () => {
    ...
    a.download = `${selected?.name ?? "gift"}-render.json`;
    a.click();
  ...
  const savePreset = async () => {
    ...
    const { error } = await supabase.from("gift_render_presets").upsert(
      { name, config: diffRenderConfig(cfg) },
      { onConflict: "name" },
    );
    if (error) return toast.error(error.message);
    toast.success("Preset saved");
    qc.invalidateQueries({ queryKey: ["admin", "gift-studio", "presets"] });
  };
      ...
      <div className="grid gap-4 lg:grid-cols-[240px_minmax(0,380px)_minmax(0,1fr)]">
        ...
        <div className="rounded-xl border border-border bg-card p-3">
          <div className="relative mb-2">
            ...
            <Input className="pl-8" placeholder="Search gifts…" value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
        ...
        <div className="space-y-3">
          <div
            ref={stageRef}
            className="relative mx-auto aspect-[9/16] w-full max-w-[340px] overflow-hidden rounded-[28px] border-4 border-neutral-800 bg-neutral-950 shadow-2xl"
          ...
          >
            ...
            <div className="absolute bottom-3 left-3 right-3 space-y-1 text-[10px] text-white/50">
              <div className="rounded bg-white/10 px-2 py-1">viewer: nice gift 🔥</div>
            ...
            {clipUrl && isVideo ? (
              ...
              <div className="absolute inset-0 grid place-items-center text-xs text-white/40">No clip on this gift</div>
            )}

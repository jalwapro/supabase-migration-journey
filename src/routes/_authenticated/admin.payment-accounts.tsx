...
function PaymentAccounts() {
  ...
  const save = useMutation({
    mutationFn: async () => {
      if (!verified) throw new Error("Google Authenticator verification is required before saving payment settings.");
  ...
  if (authLoading) return <div className="flex min-h-[300px] items-center justify-center"><Loader2 className="h-7 w-7 animate-spin text-primary" /></div>;

  if (!verified) {
    return (
      <div>
        <AdminPageHeader title="Payment Accounts" subtitle="Google Authenticator verification required for payment configuration" />
        <PaymentAccountsTwoFA onVerified={() => setVerified(true)}>
          <div />

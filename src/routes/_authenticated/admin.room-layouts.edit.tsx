...
function LayoutEditorPage() {
      ...
      if (id === 'new') {
        ...
        const { data, error } = await supabase
          ...
          .insert({
            ...
            type: 'voice', // Will be updated based on URL params
  ...
  const [currentLayout, setCurrentLayout] = useState<LayoutJSON>(
    layout?.layout_json || DEFAULT_VOICE_LAYOUT
  );

  // Update layout when data loads
  useState(() => {
    if (layout?.layout_json) {
      setCurrentLayout(layout.layout_json);
      setHistory([layout.layout_json]);
      setHistoryIndex(0);
    }
  });

...
function getLegacyStorageKey() {
  try { if (!url) return null; const projectRef = new URL(url).hostname.split(".")[0]; return projectRef ? `sb-${projectRef}-auth-token` : null; } catch { return null; }
}
...
/**
 * Studio preview is deliberately data-isolated. The real Supabase client is
 * never queried while ?adminPreview=1&previewIdentity=neutral is present.
 * This keeps the existing frontend components/renderers usable without
 * exposing production users, rooms, chats, balances, transactions or history.
 */
...
function createStudioQuery(table: string) {
  ...
  const query: any = {
    select: () => query, eq: (column: string, value: any) => { rows = rows.filter((r) => r?.[column] === value || r?.[column]?.id === value); return query; }, neq: (column: string, value: any) => { rows = rows.filter((r) => r?.[column] !== value); return query; }, in: (column: string, values: any[]) => { rows = rows.filter((r) => values.includes(r?.[column])); return query; }, ilike: () => query, like: () => query, or: () => query, and: () => query, order: () => query, limit: (n: number) => { rows = rows.slice(0, n); return query; }, range: (a: number, b: number) => { rows = rows.slice(a, b + 1); return query; }, single: async () => ({ data: rows[0] ?? null, error: null }), maybeSingle: async () => ({ data: rows[0] ?? null, error: null }), insert: async () => ({ data: rows, error: null }), update: async () => ({ data: rows, error: null }), upsert: async () => ({ data: rows, error: null }), delete: async () => ({ data: [], error: null }), then: (resolve: (value: any) => any) => Promise.resolve({ data: rows, error: null, count: rows.length }).then(resolve), catch: (reject: (reason: any) => any) => Promise.resolve({ data: rows, error: null }).catch(reject)
  };
  return query;
}
...
const studioProxy = new Proxy(realSupabase as any, {
  get(target, property) {
    ...
    if (property === "channel") return () => ({ on: () => ({ subscribe: () => ({ unsubscribe: () => {} }) }), subscribe: () => ({ unsubscribe: () => {} }), unsubscribe: async () => "ok" });
    if (property === "removeChannel") return async () => "ok";
    if (property === "removeAllChannels") return async () => "ok";
    if (property === "storage") return { from: () => ({ upload: async () => ({ data: null, error: null }), remove: async () => ({ data: null, error: null }), getPublicUrl: (path: string) => ({ data: { publicUrl: path } }), list: async () => ({ data: [], error: null }) }) };
    return target[property as keyof typeof target];
  },
});

/** In design preview this export is a data-isolated facade; production uses the untouched real client. */

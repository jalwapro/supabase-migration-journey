...
function getCookie(name: string) {
  ...
  try {
    ...
    return part ? decodeURIComponent(part.slice(encodedName.length)) : null;
  } catch { return null; }
}
...
function setCookieAuthItem(storageKey: string, value: string) {
  ...
  if (chunks > COOKIE_CHUNK_LIMIT) return;
  setCookie(`${prefix}_chunks`, String(chunks));
  for (let i = 0; i < chunks; i += 1) setCookie(`${prefix}_${i}`, value.slice(i * COOKIE_CHUNK_SIZE, (i + 1) * COOKIE_CHUNK_SIZE));
}
...
function getLocalAuthItem(storageKey: string) {
  ...
  try { return window.localStorage.getItem(storageKey) ?? memoryAuthStorage.get(storageKey) ?? null; }
  catch { return memoryAuthStorage.get(storageKey) ?? null; }
}
function setLocalAuthItem(storageKey: string, value: string) {
  memoryAuthStorage.set(storageKey, value);
  if (typeof window !== "undefined") try { window.localStorage.setItem(storageKey, value); } catch {}
}
function removeLocalAuthItem(storageKey: string) {
  memoryAuthStorage.delete(storageKey);
  if (typeof window !== "undefined") try { window.localStorage.removeItem(storageKey); } catch {}
}
...
function studioRows(table: string): any[] {
  ...
  if (["rooms", "live_rooms"].includes(table)) return Array.from({ length: 6 }, (_, i) => ({
    id: `10000000-0000-4000-8000-${String(i + 1).padStart(12, "0")}`,
    name: `Demo Voice Room ${i + 1}`,
    title: `Demo Live Room ${i + 1}`,
    host_id: profiles[i].id,
    host: profiles[i],
    status: "live",
    room_type: i % 2 ? "video" : "voice",
    member_count: 4 + i,
    viewer_count: 4 + i,
    seat_count: 12,
    max_seats: 12,
  ...
  if (["room_members", "room_participants", "room_seats"].includes(table)) return profiles.slice(0, 6).map((p, i) => ({
  ...
  if (["gifts", "gift_catalog", "gift_items"].includes(table)) return ["Rose", "Heart", "Rocket", "Crown", "Diamond", "Castle"].map((name, i) => ({
    id: `gift-${i + 1}`, name: `Demo ${name}`, title: `Demo ${name}`, price: (i + 1) * 100, coins: (i + 1) * 100, image_url: "", is_active: true, sort_order: i,
  }));
  if (["notifications", "user_notifications"].includes(table)) return [
    { id: "notice-1", title: "Welcome to Studio Preview", message: "Demo notification", read: false, created_at: now },
    { id: "notice-2", title: "Demo Event", message: "Preview-only notification", read: true, created_at: now },
  ];
  if (["messages", "conversations", "chats", "chat_messages"].includes(table)) return profiles.slice(0, 4).map((p, i) => ({
    id: `msg-${i + 1}`, user_id: p.id, profile: p, sender_id: p.id, receiver_id: MOCK_USER_ID,
    content: ["Hello 👋", "Welcome", "Nice room!", "See you live"][i], message: ["Hello 👋", "Welcome", "Nice room!", "See you live"][i], created_at: now,
  }));
  if (["followers", "following", "user_followers", "follows"].includes(table)) return profiles.slice(1, 6).map((p, i) => ({
    id: `follow-${i + 1}`, follower_id: MOCK_USER_ID, following_id: p.id, profile: p, created_at: now,
  }));
  ...
  if (["transactions", "wallet_transactions", "recharge_history", "withdrawals", "gift_history"].includes(table)) return Array.from({ length: 5 }, (_, i) => ({
    id: `txn-${i + 1}`, user_id: MOCK_USER_ID, amount: (i + 1) * 500, coins: (i + 1) * 1000, diamonds: (i + 1) * 500, status: "completed", type: i % 2 ? "recharge" : "demo", created_at: now,
  }));
  if (["banners", "home_banners"].includes(table)) return [{ id: "banner-demo-1", title: "Demo Banner", image_url: "", link_url: null, active: true, sort_order: 0 }];
  if (["categories", "room_categories"].includes(table)) return ["Popular", "Voice", "Video", "PK", "Games"].map((name, i) => ({ id: `cat-${i + 1}`, name, title: name, sort_order: i, is_active: true }));
  if (["recharge_packages", "coin_packages"].includes(table)) return [100, 500, 1000, 5000].map((coins, i) => ({ id: `pkg-${i + 1}`, coins, amount: coins / 5, currency: "PKR", is_active: true, sort_order: i }));
  return [];
}
...
function createStudioQuery(table: string) {
  ...
  const query: any = {
    ...
    not: () => query,
    is: (column: string, value: any) => { rows = rows.filter((r) => value === null ? r?.[column] == null : r?.[column] === value); return query; },
    gt: (column: string, value: any) => { rows = rows.filter((r) => r?.[column] > value); return query; },
    gte: (column: string, value: any) => { rows = rows.filter((r) => r?.[column] >= value); return query; },
    lt: (column: string, value: any) => { rows = rows.filter((r) => r?.[column] < value); return query; },
    lte: (column: string, value: any) => { rows = rows.filter((r) => r?.[column] <= value); return query; },
    ilike: () => query,
    like: () => query,
    or: () => query,
    and: () => query,
    ...
    range: (a: number, b: number) => { rows = rows.slice(a, b + 1); return query; },
    insert: () => query,
    update: () => query,
    upsert: () => query,
    delete: () => query,
...
const studioUser = {
  ...
  aud: "authenticated",
  role: "authenticated",
  email: "studio-preview@jalwa.local",
  app_metadata: {},
  user_metadata: { username: "Demo User" },
  created_at: "2026-01-01T00:00:00.000Z",
};
const studioAuth = {
  getSession: async () => ({ data: { session: { access_token: "studio-preview", refresh_token: "studio-preview", expires_in: 3600, expires_at: Math.floor(Date.now() / 1000) + 3600, token_type: "bearer", user: studioUser } }, error: null }),
  getUser: async () => ({ data: { user: studioUser }, error: null }),
  onAuthStateChange: (callback: any) => { queueMicrotask(() => callback("INITIAL_SESSION", null)); return { data: { subscription: { unsubscribe: () => {} } } }; },
  signOut: async () => ({ error: null }),
};
const studioFunctions = { invoke: async () => ({ data: null, error: null }) };
const studioStorage = { from: () => ({ upload: async () => ({ data: null, error: null }), remove: async () => ({ data: null, error: null }), getPublicUrl: (path: string) => ({ data: { publicUrl: path } }), list: async () => ({ data: [], error: null }) }) };
const studioRealtimeChannel = () => ({ on: () => ({ subscribe: () => ({ unsubscribe: () => {} }) }), subscribe: () => ({ unsubscribe: () => {} }), unsubscribe: async () => "ok" });

const studioProxy = new Proxy(realSupabase as any, {
  get(target, property) {
    if (!isStudioPreview()) return target[property as keyof typeof target];
    if (property === "from") return (table: string) => createStudioQuery(table);
    if (property === "auth") return studioAuth;
    if (property === "rpc") return async () => ({ data: null, error: null });
    if (property === "functions") return studioFunctions;
    if (property === "channel") return studioRealtimeChannel;
    if (property === "removeChannel") return async () => "ok";
    if (property === "removeAllChannels") return async () => "ok";
    if (property === "storage") return studioStorage;
    return target[property as keyof typeof target];
  },
});

// Hard network boundary for preview windows. This is intentionally dynamic so
// a route redirect cannot accidentally restore the Admin session's API access.
if (typeof window !== "undefined") {
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
    if (isStudioPreview()) {
      ...
      if (sameOriginApi || supabaseApi) {
        return new Response(JSON.stringify({ data: [], error: null, mock: true }), { status: 200, headers: { "content-type": "application/json" } });
      }
    }
...
// IMPORTANT: evaluate preview mode at request time, not once when the module
// is imported. The Studio iframe navigates between real app routes; the
// window-name preview marker keeps the mock boundary active across redirects.
export const supabase: SupabaseClient = studioProxy as SupabaseClient;

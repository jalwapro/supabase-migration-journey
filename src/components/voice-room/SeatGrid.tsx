  ...
  if (!user) {
    return (
      <div className={cn("relative flex flex-col items-center justify-center p-0.5", className)}>
        ...
        >
          <span className="absolute left-0.5 top-0.5 z-10 grid h-4 w-4 place-items-center rounded-full bg-black/70 text-[9px] font-bold text-white/90">
            {num}
          </span>
          {/* DP Size Increased to w-[52px] */}
          <span className="relative aspect-square w-[52px] max-w-full rounded-full p-0">
            <EmptySeatArt locked={is_locked} />
          </span>
          <span className="w-full truncate text-[9px] font-semibold text-white/70 text-center">No.{num}</span>
  ...
  return (
    <div className={cn("relative flex flex-col items-center justify-center p-0.5", className)}>
      <button
        ...
        className="group relative flex w-full flex-col items-center justify-center gap-0.5 bg-transparent p-0 transition-all active:scale-95 border-0 shadow-none cursor-pointer"
      >
        <span className="absolute left-0.5 top-0.5 z-10 grid h-4 w-4 place-items-center rounded-full bg-black/75 text-[9px] font-black text-white">
          {num}
        </span>
        {/* DP Size Increased to w-[52px] */}
        <span className="relative aspect-square w-[52px] max-w-full p-0 flex items-center justify-center my-0.5">
          <span className={cn(
          ...
          <span className={cn("absolute -bottom-0.5 -right-0.5 z-40 grid h-3.5 w-3.5 place-items-center rounded-full border border-black text-white shadow-md", isMuted ? "bg-red-500" : "bg-emerald-500")}>
...
export function SeatGrid({ seats, seatCount, host, roomId, isHost = false, onSeatTap, onJoinSeat, onHostTap }: SeatGridProps) {
  ...
  return (
    <section className="relative flex w-full flex-col items-center px-1 py-0.5" data-seat-capacity={capacity}>
      {/* Container height optimized for bigger seats */}
      <div className="grid w-full grid-cols-5 gap-1.5 content-start max-h-[50svh] min-h-[160px] overflow-y-auto px-0.5 pb-2 scrollbar-thin scrollbar-thumb-amber-500/25">
        <div className="relative flex flex-col items-center justify-center p-0.5">
          <div className="group relative flex w-full flex-col items-center justify-center gap-0.5 bg-transparent p-0">
            <span className="absolute left-0.5 top-0.5 z-10 grid h-4 w-4 place-items-center rounded-full bg-black/80 text-[9px] font-black text-amber-300">
              1
            </span>
            {/* Host DP Size Increased to w-[52px] */}
            <span className="relative aspect-square w-[52px] max-w-full p-0 flex items-center justify-center my-0.5">

...
export function VoiceRoomGiftSheet({ open, onOpenChange, receiverName, onSendGift }: VoiceRoomGiftSheetProps) {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="rounded-t-3xl border-white/10 bg-[#0b0710] p-0 text-white" data-adaptive="neon">
        ...
        <div className="grid max-h-[45vh] grid-cols-4 gap-2 overflow-y-auto px-4 py-2 sm:grid-cols-6">
          {gifts.map((g) => (
            <button
              key={g.id}
            ...
            >
              ...
              <span className="truncate text-[10px] text-white/75">{g.name}</span>
              <span className="text-[10px] font-semibold text-amber-300">{g.price}</span>
            </button>
          ))}
        </div>
        ...
        <div className="flex items-center justify-between gap-3 border-t border-white/10 px-4 py-3">
          <div className="flex items-center gap-2 text-xs text-white/60">
            ...
            {selected.name} · <span className="font-semibold text-amber-300">{selected.price} coins</span>
          </div>
          <button
            onClick={() => onSendGift(selected)}
            className="rounded-full bg-gradient-to-r from-fuchsia-500 to-violet-600 px-5 py-2 text-xs font-bold text-white shadow-[0_0_16px_-4px_rgba(232,60,220,0.8)] transition-transform active:scale-95"
          >
            Send Gift
          </button>
        </div>

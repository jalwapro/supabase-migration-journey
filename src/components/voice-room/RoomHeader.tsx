...
export function RoomHeader({ room, roomCode, onlineCount, topGifterName, topGifterCoins, onHostTap, onReport, onShare, onExit, onHome, onRanking, isHost: isHostProp = false }: RoomHeaderProps) {
  ...
  const inviteToSeat = async (memberId: string) => {
    ...
    if (pendingInvites.has(memberId)) { toast.info("Seat invite already sent"); return; }
    ...
    try {
      ...
      if (error) throw error;
      setPendingInvites(prev => new Set(prev).add(memberId));
      const target = members.find(m => m.id === memberId);
      toast.success(`Seat invite sent to ${target?.username ?? "user"}`);
    } catch (error) {
      toast.error(`Unable to send seat invite: ${(error as Error).message || "Unknown error"}`);

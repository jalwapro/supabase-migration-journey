...
export function RoomHeader({ room, roomCode, onlineCount, topGifterName, topGifterCoins, onHostTap, onReport, onShare, onExit, onHome, onRanking }: RoomHeaderProps) {
  ...
  return <header className="relative z-[60] flex shrink-0 flex-col px-2 pt-[calc(.2rem+env(safe-area-inset-top))] sm:px-2.5" style={{ pointerEvents: "auto" }}>
    <div className="relative z-[61] flex min-h-[56px] items-center gap-2">
      <button type="button" onClick={onHostTap ? tap(onHostTap) : tap(onHome)} className="relative z-[62] flex min-w-0 max-w-[66%] flex-1 items-center gap-2 rounded-[18px] border border-white/10 bg-[linear-gradient(135deg,rgba(27,45,76,.96),rgba(19,29,52,.96))] p-2 text-left shadow-[inset_0_1px_rgba(255,255,255,.06)] active:opacity-85" aria-label={`Open ${hostName} profile`}>
        {room.host.avatar ? <img src={room.host.avatar} alt="" className="h-11 w-11 shrink-0 rounded-[12px] border border-cyan-100/30 object-cover" /> : <div className="grid h-11 w-11 shrink-0 place-items-center rounded-[12px] border border-cyan-100/30 bg-white/10 text-sm font-black">{hostName.charAt(0).toUpperCase()}</div>}
        <div className="min-w-0"><div className="truncate text-[14px] font-bold text-white">{room.title}</div><div className="mt-1 flex items-center gap-1.5 text-[10px] text-white/55"><span className="inline-grid h-4 w-4 place-items-center rounded-full bg-amber-500/20 text-[9px] text-amber-300">◆</span><span className="truncate">ID:{roomCode}</span></div></div>
      </button>

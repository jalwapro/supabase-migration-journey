...
export const VoiceRoomScreen = ({
  ...
  useEffect(() => {
    ...
    const fetchSlides = async () => {
      const { data, error } = await supabase.from("room_slides").select("id, title, image_url, link_url").eq("is_active", true).order("sort_order", { ascending: true });
    ...
    void fetchSlides();
    const channel = supabase.channel("room-slides-sync").on("postgres_changes", { event: "*", schema: "public", table: "room_slides" }, () => void fetchSlides()).subscribe();
    return () => { active = false; void supabase.removeChannel(channel); };
  }, []);

  useEffect(() => {
    if (gameSlides.length <= 1) return;
    const slideInterval = setInterval(() => setCurrentSlideIndex(prev => (prev + 1) % gameSlides.length), 5000);
    return () => clearInterval(slideInterval);
  }, [gameSlides.length]);

  useEffect(() => {
    if (popularityPct >= 100 && isHost) setShowHostFreePopup(true);
  ...
  useEffect(() => {
    ...
    const syncRole = async () => {
      ...
      if (!auth.user) { if (active) setIsModerator(false); return; }
  ...
  useEffect(() => {
    ...
    const syncSettings = async () => {
      ...
      if (active && !error && data) {
        ...
        if (data.seat_count != null) setLiveSeatCount(normalizeCapacity(data.seat_count));
      }
    };
    void syncSettings();
  ...
  const openMoreForRole = () => {
    if (isHost) setHostSettingsOpen(true);
    else if (isModerator) setModeratorControlsOpen(true);
    else setUserMoreOpen(true);
  };

  const joinSeat = (index: number) => {
    if (roomSettings.is_locked && !isHost && !isUnlockedByPin) { setShowPinModal(true); return; }
    onJoinSeat(index);
  };
  ...
  // FIXED: Seat click automatically populates @username mention in chat input draft
  const openMemberProfile = (seatIndex: number) => {
    const seat = room.seats.find(s => s.index === seatIndex);
    if (!seat?.user) return;
    
    setMiniDraft(`@${seat.user.username} `);
  ...
  return (
    <main className="fixed inset-0 z-50 mx-auto flex w-full max-w-[480px] flex-col overflow-hidden bg-slate-950 text-foreground shadow-2xl h-[100svh]">
        ...
        <div className="absolute inset-0 z-[100] flex items-center justify-center bg-black/85 backdrop-blur-md px-4 animate-fade-in">
          <div className="relative flex flex-col items-center max-w-sm w-full rounded-3xl bg-slate-900 border border-amber-500/50 p-6 shadow-[0_0_35px_rgba(234,179,8,0.3)] text-center">
            ...
            <div className="mt-6 flex gap-3 w-full">
              <button type="button" onClick={() => { if (enteredPin.trim() === roomSettings.room_pin) { setIsUnlockedByPin(true); setShowPinModal(false); } else { alert("Incorrect Room PIN! Please try again."); } }} className="flex-1 py-3 rounded-xl bg-gradient-to-r from-amber-500 to-purple-600 text-white font-bold text-xs shadow-lg active:scale-95 transition-transform">Enter Room ✨</button>
              <button type="button" onClick={() => { setShowPinModal(false); onExit(); }} className="px-4 py-3 rounded-xl bg-white/10 text-slate-300 font-semibold text-xs active:scale-95 transition-transform">Leave</button>
            </div>
          </div>
        </div>
      )}
      ...
      {generatedPin && isHost && (
        <div className="absolute inset-0 z-[90] flex items-center justify-center bg-black/75 backdrop-blur-md px-4 animate-fade-in">
          <div className="relative flex flex-col items-center max-w-sm w-full rounded-3xl bg-slate-900 border border-purple-500/50 p-6 shadow-[0_0_35px_rgba(168,85,247,0.4)] text-center">
            <div className="h-14 w-14 rounded-full bg-purple-500/20 border border-purple-400 flex items-center justify-center mb-3 text-purple-300"><KeyRound className="h-7 w-7" /></div>
            ...
            <div className="my-4 px-6 py-2 rounded-xl bg-black/80 border border-amber-400/60 text-amber-300 font-black text-2xl tracking-widest shadow-inner">{generatedPin}</div>
            <button type="button" onClick={() => setGeneratedPin(null)} className="w-full py-3 rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold text-xs shadow-lg active:scale-95 transition-transform">Okay, Got It 👍</button>
          </div>
        </div>
      )}
            ...
            <section className="grid grid-cols-[minmax(0,1.7fr)_minmax(85px,.7fr)] gap-1.5">
              ...
              <div className="flex min-w-0 flex-col justify-between items-end gap-1 max-h-[190px]">
                <button type="button" onClick={tap(openPopularity)} style={{ height: "75px" }} className={cn(buttonClass, "relative group flex items-center justify-center bg-transparent border-0 p-0 w-full shrink-0 shadow-none")}>
                  <div className="relative h-[75px] w-[75px] flex items-center justify-center"><img src="/images/jalwa-1.gif" alt="Rocket GIF" className="h-[75px] w-[75px] object-contain animate-bounce drop-shadow-[0_0_10px_rgba(234,179,8,0.5)]" /></div>
                </button>
      ...
      <nav style={{ backgroundColor: "rgba(10, 10, 15, 0.95)" }} className="relative z-20 flex h-[52px] shrink-0 items-center justify-around border-t border-white/15 px-1 pb-[env(safe-area-inset-bottom)] backdrop-blur-xl">
        ...
        <button type="button" onClick={tap(onOpenPrivateChat)} className={cn(buttonClass,"grid h-9 w-9 place-items-center rounded-full border border-cyan-400/50 bg-cyan-500/20 text-cyan-300 shadow-md")} aria-label="Private Chat"><MessageCircle className="h-4 w-4" /></button>
      ...
      {/* Keyboard-attached mini input popup modal with url inputMode to clean samsung toolbar */}
      {showMiniInputPopup && (
        <div className="absolute inset-0 z-[2147483500] flex items-end justify-center bg-black/60 backdrop-blur-sm animate-fade-in p-0" onClick={() => setShowMiniInputPopup(false)}>
          <div className="w-full max-w-[480px] bg-[#120a1f] border-t border-amber-500/40 p-2.5 rounded-t-2xl shadow-2xl flex items-center gap-2 animate-slide-up" onClick={e => e.stopPropagation()}>
            <input
              value={miniDraft}
            ...
            />
            <button type="button" onClick={async () => { if (!miniDraft.trim()) return; setDraft(miniDraft); setShowMiniInputPopup(false); setMiniDraft(""); await sendRoomMessage(); }} className="px-4 py-2 rounded-xl bg-gradient-to-r from-amber-400 to-purple-600 text-black font-extrabold text-xs shadow-lg active:scale-95 transition">Send</button>
          </div>
        </div>
      )}
        ...
        <div className="absolute inset-0 z-50 flex flex-col justify-end bg-black/60 backdrop-blur-sm animate-fade-in">
          ...
          <div className="relative z-10 w-full rounded-t-3xl bg-slate-900/95 border-t border-amber-500/30 p-4 shadow-2xl backdrop-blur-xl animate-slide-up max-h-[60dvh] flex flex-col">
            ...
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <div className="flex items-center gap-2"><Smile className="h-4 w-4 text-amber-400" /><span className="text-xs font-bold tracking-wide text-amber-400 uppercase">Send Emoji to Seat</span></div>
        ...
        <div className="fixed inset-0 z-[2147483000] flex items-end justify-center bg-black/60 p-0 backdrop-blur-sm animate-fade-in" onClick={() => setUserMoreOpen(false)}>
          <div className="w-full max-w-sm rounded-t-3xl border-t border-x border-white/20 bg-[#100719]/95 p-4 text-white shadow-2xl backdrop-blur-md flex flex-col animate-slide-up" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between pb-3 border-b border-white/10">
              <div className="flex items-center gap-2"><Info className="h-4 w-4 text-cyan-400" /><h3 className="text-xs font-black uppercase tracking-wider text-cyan-300">Room Options</h3></div>
              <button type="button" onClick={() => setUserMoreOpen(false)} className="p-1 rounded-full bg-white/10 text-white"><X className="h-4 w-4" /></button>
            </div>

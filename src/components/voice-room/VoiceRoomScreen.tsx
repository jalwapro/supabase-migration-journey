...
import { Rocket, Gift, Gamepad2, Smile, Send, Mail, Grid2X2, Mic, MicOff, Volume2, VolumeX } from "lucide-react";
...
interface VoiceRoomScreenProps {
  room: RoomState; roomId: string; seatCount?: number; roomCode: string; onlineCount: number; micOn: boolean; isHost: boolean; mySeatIndex: number | null; popularityPct: number;
  topGifterName?: string | null; topGifterCoins?: number; announcement?: string | null; messages?: RoomChatMessage[];
  onOpenChat: () => void; onOpenPrivateChat: () => void; onOpenGift: () => void; onOpenMore: () => void; onToggleMic: () => void;
  speakerMuted: boolean; onToggleSpeaker: () => void; onOpenMusic?: () => void; onSendEmoji?: (emoji: ChatEmoji) => void; canPlayMusic?: boolean;
  onSeatTap: (i: number) => void; onJoinSeat: (i: number) => void; onHostTap?: () => void; onReport: () => void; onShare: () => void;
  onExit: () => void; onHome: () => void; onRanking: () => void; onOpenGames: () => void;
}

export const VoiceRoomScreen = ({ room, roomId, seatCount, roomCode, onlineCount, micOn, speakerMuted, isHost, mySeatIndex, popularityPct, topGifterName, topGifterCoins, announcement, messages = [], onOpenChat, onOpenPrivateChat, onOpenGift, onOpenMore, onToggleMic, onToggleSpeaker, onOpenMusic, onSendEmoji, canPlayMusic, onSeatTap, onJoinSeat, onHostTap, onReport, onShare, onExit, onHome, onRanking, onOpenGames }: VoiceRoomScreenProps) => {
  ...
  useEffect(() => setRoomMessages(messages), [messages]);

  useEffect(() => {
    let active = true;
    const channelName = `voice-chat-${roomId}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    const load = async () => {
      const { data } = await supabase.from("room_messages").select("id,user_id,kind,text,message,created_at,sender_username,sender_avatar").eq("room_id", roomId).neq("kind", "emoji").order("created_at", { ascending: false }).limit(50);
      if (active && data) setRoomMessages(data as RoomChatMessage[]);
    };
    void load();
    const ch = supabase.channel(channelName)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "room_messages", filter: `room_id=eq.${roomId}` }, p => { const row = p.new as RoomChatMessage; if (row.kind === "emoji") return; setRoomMessages(prev => prev.some(m => m.id === row.id) ? prev : [...prev, row].slice(-50)); })
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "room_messages", filter: `room_id=eq.${roomId}` }, p => { const row = p.new as RoomChatMessage; if (row.kind === "emoji") return; setRoomMessages(prev => prev.some(m => m.id === row.id) ? prev.map(m => m.id === row.id ? { ...m, ...row } : m) : [...prev, row].slice(-50)); })
      .on("postgres_changes", { event: "DELETE", schema: "public", table: "room_messages", filter: `room_id=eq.${roomId}` }, p => { const row = p.old as { id: string }; setRoomMessages(prev => prev.filter(m => m.id !== row.id)); });
    void ch.subscribe();
  ...
  useEffect(() => {
    ...
    let active = true;
    void supabase.from("live_rooms").select("seat_count").eq("id", roomId).maybeSingle().then(({ data, error }) => { if (active && !error && data) setLiveSeatCount(normalizeCapacity((data as { seat_count?: number }).seat_count)); });
    const ch = supabase.channel(`voice-layout-${roomId}-${Date.now()}`).on("postgres_changes", { event: "UPDATE", schema: "public", table: "live_rooms", filter: `id=eq.${roomId}` }, p => { const r = p.new as { seat_count?: number }; if (r.seat_count != null) setLiveSeatCount(normalizeCapacity(r.seat_count)); }).subscribe();
    return () => { active = false; void supabase.removeChannel(ch); };
  }, [roomId, seatCount]);
  ...
  useEffect(() => {
    ...
    if (!hostId) { setHostTheme(null); return; }
    void supabase.from("profiles").select("theme_id").eq("id", hostId).maybeSingle().then(async ({ data, error }) => {
      if (cancelled) return;
      if (error || !data?.theme_id) { setHostTheme(null); return; }
  ...
  useEffect(() => {
    ...
    const previousPrimary = document.body.style.getPropertyValue("--primary");
    const previousSecondary = document.body.style.getPropertyValue("--secondary");
    ...
    return () => {
      if (previousPrimary) document.body.style.setProperty("--primary", previousPrimary); else document.body.style.removeProperty("--primary");
      if (previousSecondary) document.body.style.setProperty("--secondary", previousSecondary); else document.body.style.removeProperty("--secondary");
    };
  }, [hostTheme]);
  ...
  const receivers = useMemo<GiftReceiver[]>(() => { const s = room.seats.filter(x => x.user && x.user.id !== room.host.id).map(x => ({ id: x.user!.id, username: x.user!.username, avatar: x.user!.avatar })); return room.host.id ? [{ id: room.host.id, username: room.host.username, avatar: room.host.avatar }, ...s] : s; }, [room]);
      ...
      <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden">
        ...
        <section className="grid h-[clamp(160px,27dvh,220px)] shrink-0 grid-cols-[minmax(0,1.7fr)_minmax(80px,.7fr)] gap-1.5 px-2 py-1.5">
          <div className="flex min-w-0 min-h-0 flex-col overflow-hidden rounded-[14px] border border-white/45 bg-transparent">
            ...
            <button type="button" onClick={tap(onOpenChat)} className={cn(buttonClass,"flex min-h-0 flex-1 flex-col overflow-hidden px-2.5 py-1.5 text-left")}><div className="min-h-0 flex-1 space-y-1 overflow-hidden">{visibleMessages.length===0?<p className="rounded-xl border border-white/30 px-2.5 py-1.5 text-[11px] text-white/85 drop-shadow-[0_1px_3px_rgba(0,0,0,.9)]">{announcement||"Say hi to break the ice 👋"}</p>:visibleMessages.map(m=><div key={m.id} className="flex min-w-0 items-start gap-1.5 drop-shadow-[0_1px_3px_rgba(0,0,0,.9)]"><span className="shrink-0 text-[10px] font-bold text-[color:var(--primary)]">{m.user?.username||m.sender_username||"User"}:</span><span className="min-w-0 break-words text-[11px] text-white">{m.text||m.message||""}</span></div>)}</div></button>
            <button type="button" onClick={tap(onOpenChat)} className={cn(buttonClass,"mx-1.5 mb-1.5 flex h-9 shrink-0 items-center gap-2 rounded-full border border-white/50 bg-white/10 px-2.5 text-left shadow-[0_1px_8px_rgba(0,0,0,.25)]")}><span className="min-w-0 flex-1 truncate text-xs text-white/85 drop-shadow-[0_1px_3px_rgba(0,0,0,.9)]">Say something...</span><Smile className="h-3.5 w-3.5 shrink-0 text-white/75"/><Send className="h-3.5 w-3.5 shrink-0 text-[color:var(--primary)]"/></button>
          </div>
          <div className="flex min-w-0 flex-col justify-center gap-1.5"><button type="button" onClick={tap(onRanking)} className={cn(buttonClass,"grid min-h-0 flex-1 place-items-center rounded-xl border border-white/45 bg-white/10 p-1.5 shadow-[0_1px_8px_rgba(0,0,0,.25)]")}><span className="flex flex-col items-center drop-shadow-[0_1px_3px_rgba(0,0,0,.9)]"><Rocket className="h-7 w-7 text-[color:var(--secondary)]"/><span className="mt-0.5 text-[9px] font-bold text-white/90">{popularityPct}% Popular</span></span></button><button type="button" onClick={tap(openGames)} className={cn(buttonClass,"h-10 shrink-0 overflow-hidden rounded-xl border border-white/45 bg-white/10 px-1 text-[10px] font-black text-white shadow-[0_1px_8px_rgba(0,0,0,.25)]")}>Games</button></div>
        </section>
      </div>
    </div>

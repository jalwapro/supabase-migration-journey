...
import { Rocket, Gift, Gamepad2, Smile, Send, Grid2X2, Mic, MicOff, Volume2, VolumeX, Shield } from "lucide-react";
...
export const VoiceRoomScreen = ({ room, roomId, seatCount, roomCode, onlineCount, micOn, speakerMuted, isHost, mySeatIndex, popularityPct, topGifterName, topGifterCoins, announcement, messages = [], onOpenChat, onOpenGift, onOpenMore, onToggleMic, onToggleSpeaker, onSendEmoji, onSeatTap, onJoinSeat, onHostTap, onReport, onShare, onExit, onHome, onRanking, onOpenGames }: VoiceRoomScreenProps) => {
  ...
  useEffect(() => {
    ...
    void syncRole();
    const channel = supabase.channel(`voice-role-${roomId}`)
  ...
  useEffect(() => {
    ...
    void syncSettings();
    const channel = supabase.channel(`voice-room-settings-${roomId}`)
  ...
  const receivers = useMemo<GiftReceiver[]>(() => room.seats.filter(x => x.user).map(x => ({ id: x.user!.id, username: x.user!.username, avatar: x.user!.avatar })), [room.seats]);
  ...
  return <main className="relative z-50 mx-auto flex h-[100dvh] min-h-0 w-full max-w-none flex-col overflow-hidden bg-background text-foreground shadow-2xl">
    ...
    <div className="relative z-10 flex min-h-0 flex-1 flex-col overflow-hidden bg-transparent">
      ...
      <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden">
        <div className="relative min-h-0 flex-1 overflow-hidden bg-transparent pt-1"><SeatGrid seats={room.seats} seatCount={effectiveSeatCount} host={room.host} roomId={roomId} isHost={isHost} onSeatTap={openMemberProfile} onJoinSeat={joinSeat} onHostTap={onHostTap} /></div>
        <section className="grid h-[clamp(160px,27dvh,220px)] shrink-0 grid-cols-[minmax(0,1.7fr)_minmax(80px,.7fr)] gap-1.5 px-2 py-1.5">
          <div className="flex min-w-0 min-h-0 flex-col overflow-hidden rounded-[14px] border border-white/45 bg-white/5 backdrop-blur-md">
            <div className="flex h-8 shrink-0 items-end gap-5 border-b border-white/30 px-2.5"><button type="button" onClick={tap(onOpenChat)} disabled={!roomSettings.chat_enabled} className={cn(buttonClass,"relative pb-1.5 text-xs font-semibold text-white disabled:opacity-40")}>All<span className="absolute bottom-0 left-0 h-0.5 w-6 rounded-full bg-[color:var(--primary)]" /></button><button type="button" onClick={tap(onOpenChat)} disabled={!roomSettings.chat_enabled} className={cn(buttonClass,"pb-1.5 text-xs font-medium text-white/75 disabled:opacity-40")}>Chat</button></div>
            <div className="min-h-0 flex-1 overflow-y-auto px-2.5 py-1.5">{visibleMessages.length === 0 ? <div className="flex h-full items-center justify-center text-center text-white/45"><div><div className="mx-auto mb-1 grid h-7 w-7 place-items-center rounded-full bg-white/10"><Smile className="h-4 w-4" /></div><p className="text-[11px] font-medium">{announcement || "No messages yet"}</p><p className="text-[10px]">{roomSettings.chat_enabled ? "Start the conversation" : "Chat is disabled by the Host"}</p></div></div> : visibleMessages.map(m => { const avatar = m.user?.avatar || m.sender_avatar; const fallback = (m.user?.username || m.sender_username || "U").trim().charAt(0).toUpperCase(); return <div key={m.id} className="mb-1 flex min-w-0 items-start gap-1.5 text-xs"><div className="grid h-5 w-5 shrink-0 overflow-hidden rounded-full bg-white/20 text-[9px] font-bold text-white/80 place-items-center">{avatar ? <img src={avatar} alt="" className="h-full w-full object-cover" onError={e => { e.currentTarget.style.display = "none"; }} /> : fallback}</div><span className="min-w-0 break-words text-black">{m.text || m.message || ""}</span>{m.pending && <span className="text-[9px] text-slate-500">sending…</span>}{m.failed && <span className="text-[9px] text-red-600">failed</span>}</div>; })}</div>
            <form onSubmit={e => { e.preventDefault(); void sendRoomMessage(); }} className={cn("mx-1.5 mb-1.5 flex h-10 shrink-0 items-center gap-2 rounded-full border border-white/30 bg-white px-2.5 shadow-lg", !roomSettings.chat_enabled && "opacity-50")}><input value={draft} onChange={e => setDraft(e.target.value.slice(0, 500))} onKeyDown={e => { if (e.key === "Enter") { e.preventDefault(); void sendRoomMessage(); } }} placeholder={roomSettings.chat_enabled ? "Say something..." : "Chat disabled"} aria-label="Room chat message" disabled={sending || !roomSettings.chat_enabled} className="min-w-0 flex-1 bg-transparent text-xs font-medium text-black outline-none placeholder:text-slate-400" /><button type="button" onClick={openAnimatedEmojis} disabled={!roomSettings.chat_enabled} aria-label="Composer emoji" className="grid h-7 w-7 shrink-0 place-items-center rounded-full text-[color:var(--secondary)] disabled:opacity-40"><Smile className="h-4 w-4" /></button><button type="submit" disabled={!draft.trim() || sending || !roomSettings.chat_enabled} aria-label="Send message" className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-[color:var(--primary)] text-white disabled:opacity-40"><Send className="h-3.5 w-3.5" /></button></form>
          </div>
    ...
    <nav style={{ backgroundColor: "var(--primary)" }} className="relative z-20 flex h-[clamp(52px,7dvh,62px)] shrink-0 items-center justify-around border-t border-white/70 px-1 pb-[env(safe-area-inset-bottom)]"><button type="button" onClick={tap(onToggleMic)} disabled={!isHost && !roomSettings.guest_mic_enabled} className={cn(buttonClass,"grid h-9 w-9 place-items-center rounded-full border border-white/75 bg-white/15 text-white disabled:opacity-40")} aria-label={micOn ? "Mute microphone" : "Unmute microphone"}>{micOn ? <Mic className="h-4.5 w-4.5" /> : <MicOff className="h-4.5 w-4.5" />}</button><button type="button" onClick={tap(onToggleSpeaker)} className={cn(buttonClass,"grid h-9 w-9 place-items-center rounded-full border border-white/75 bg-white/15 text-white")} aria-label={speakerMuted ? "Unmute speaker" : "Mute speaker"}>{speakerMuted ? <VolumeX className="h-4 w-4" /> : <Volume2 className="h-4 w-4" />}</button><button type="button" onClick={openAnimatedEmojis} className={cn(buttonClass,"relative z-30 grid h-10 w-10 shrink-0 place-items-center rounded-full border-2 border-white/90 bg-white/20 text-white shadow-lg ring-1 ring-black/10")} aria-label="Animated Emojis" aria-expanded={emojiOpen}><Smile className="h-5 w-5" /></button><button type="button" onClick={tap(openGifts)} disabled={!roomSettings.gifts_enabled} className={cn(buttonClass,"grid h-11 w-11 place-items-center rounded-full border-2 border-white bg-white/20 text-white disabled:opacity-40")} aria-label={roomSettings.gifts_enabled ? "Gifts" : "Gifts disabled"}><Gift className="h-5 w-5" /></button><button type="button" onClick={tap(openGames)} className={cn(buttonClass,"grid h-9 w-9 place-items-center rounded-full border border-white/75 bg-white/15 text-white")} aria-label="Games"><Gamepad2 className="h-4 w-4" /></button><button type="button" onClick={openMoreForRole} className={cn(buttonClass,"grid h-9 w-9 place-items-center rounded-full border border-white/75 bg-white/15 text-white")} aria-label={isHost ? "Host Room Settings" : isModerator ? "Moderator Controls" : "More"}>{isHost ? <Shield className="h-4 w-4" /> : isModerator ? <Shield className="h-4 w-4" /> : <Grid2X2 className="h-4 w-4" />}</button></nav>

...
export const VoiceRoomScreen = ({ room, roomId, seatCount, roomCode, onlineCount, micOn, isHost, mySeatIndex, popularityPct, topGifterName, topGifterCoins, announcement, messages = [], onOpenChat, onOpenPrivateChat, onOpenGift, onOpenMore, onToggleMic, speakerMuted, onToggleSpeaker, onSendEmoji, onSeatTap, onJoinSeat, onHostTap, onReport, onShare, onExit, onHome, onRanking, onOpenGames, onOpenNativeGame }: VoiceRoomScreenProps) => {
  ...
  const [showPinModal, setShowPinModal] = useState(false), [enteredPin, setEnteredPin] = useState(""), [isUnlockedByPin, setIsUnlockedByPin] = useState(false), [generatedPin, setGeneratedPin] = useState<string | null>(null);
  const [selectedTargetSeat, setSelectedTargetSeat] = useState<number | null>(null), [activeFlyingEmoji, setActiveFlyingEmoji] = useState<{ emojiUrl: string; fromSeat: number; toSeat: number } | null>(null), [glowingSeatIndex, setGlowingSeatIndex] = useState<number | null>(null), [gameSlides, setGameSlides] = useState<GameSlide[]>([]), [currentSlideIndex, setCurrentSlideIndex] = useState(0);
  const chatContainerRef = useRef<HTMLDivElement>(null), effectiveSeatCount = normalizeCapacity(seatCount ?? liveSeatCount);
  ...
  useEffect(() => { 
    const handlePopState = (e: PopStateEvent) => { 
      ...
      if (closedSomething) { e.preventDefault(); window.history.pushState(null, "", window.location.href); } 
    }; 
    window.history.pushState(null, "", window.location.href); 
    window.addEventListener("popstate", handlePopState); 
    return () => window.removeEventListener("popstate", handlePopState); 
  }, [privateChatOpen, giftOpen, emojiOpen, gamesOpen, popularityOpen, userMoreOpen, showMiniInputPopup, selectedMember, hostSettingsOpen, moderatorControlsOpen, generatedPin]);
  ...
  useEffect(() => { if (!isModerator) setModeratorControlsOpen(false); }, [isModerator]); useEffect(() => { if (!isHost) setHostSettingsOpen(false); }, [isHost]);
  ...
  const sendRoomMessage = async () => { 
    ...
    if (error) { 
      ...
      setDraft(value); 
    } else if (data) setRoomMessages(prev => [...prev.filter(m => m.id !== localId), data as RoomChatMessage].slice(-50)); 
    setSending(false); 
  };
  ...
  const receivers = useMemo<GiftReceiver[]>(() => { const list: GiftReceiver[] = []; if (room.host?.id) list.push({ id: room.host.id, username: room.host.username ?? "Host", avatar: room.host.avatar ?? null }); for (const seat of room.seats) { if (!seat.user?.id || seat.user.id === room.host.id) continue; list.push({ id: seat.user.id, username: seat.user.username, avatar: seat.user.avatar }); } const seen = new Set<string>(); return list.filter(u => !seen.has(u.id) && seen.add(u.id)); }, [room.host, room.seats]);
  ...
  const handlePickEmoji = (emoji: ChatEmoji) => { const targetSeat = selectedTargetSeat, senderSeat = mySeatIndex ?? 0; onSendEmoji?.(emoji, targetSeat); setEmojiOpen(false); if (targetSeat !== null && targetSeat !== undefined) { setActiveFlyingEmoji({ emojiUrl: emoji.url || "", fromSeat: senderSeat, toSeat: targetSeat }); setGlowingSeatIndex(targetSeat); setTimeout(() => setActiveFlyingEmoji(null), 1500); setTimeout(() => setGlowingSeatIndex(null), 3000); } };
  ...
  return (
    <main className="fixed inset-0 z-50 mx-auto flex w-full max-w-[480px] flex-col overflow-hidden bg-slate-950 text-foreground shadow-2xl h-[100svh]">
      ...
      {showPinModal && (<div className="absolute inset-0 z-[100] flex items-center justify-center bg-black/85 backdrop-blur-md px-4 animate-fade-in"><div className="relative flex flex-col items-center max-w-sm w-full rounded-3xl bg-slate-900 border border-amber-500/50 p-6 shadow-[0_0_35px_rgba(234,179,8,0.3)] text-center"><div className="h-16 w-16 rounded-full bg-amber-500/20 border border-amber-500 flex items-center justify-center mb-4 text-amber-400"><Lock className="h-8 w-8 animate-pulse" /></div><h3 className="text-lg font-black text-amber-300">Room is Locked! 🔒</h3><p className="mt-1 text-xs text-slate-300">Yeh room lock hai. Enter karne ke liye Host ka diya hua code enter karein:</p><input type="password" maxLength={6} value={enteredPin} onChange={e => setEnteredPin(e.target.value)} placeholder="Enter Room PIN..." className="mt-4 w-full px-4 py-3 rounded-xl bg-black/60 border border-white/20 text-center text-white tracking-widest font-bold text-lg outline-none focus:border-amber-400" /><div className="mt-6 flex gap-3 w-full"><button type="button" onClick={() => { if (enteredPin.trim() === roomSettings.room_pin) { setIsUnlockedByPin(true); setShowPinModal(false); } else { alert("Incorrect Room PIN! Please try again."); } }} className="flex-1 py-3 rounded-xl bg-gradient-to-r from-amber-500 to-purple-600 text-white font-bold text-xs shadow-lg active:scale-95 transition-transform">Enter Room ✨</button><button type="button" onClick={() => { setShowPinModal(false); onHome(); }} className="px-4 py-3 rounded-xl bg-white/10 text-slate-300 font-semibold text-xs active:scale-95 transition-transform">Leave</button></div></div></div>)}
      {generatedPin && isHost && (<div className="absolute inset-0 z-[90] flex items-center justify-center bg-black/75 backdrop-blur-md px-4 animate-fade-in"><div className="relative flex flex-col items-center max-w-sm w-full rounded-3xl bg-slate-900 border border-purple-500/50 p-6 shadow-[0_0_35px_rgba(168,85,247,0.4)] text-center"><div className="h-14 w-14 rounded-full bg-purple-500/20 border border-purple-400 flex items-center justify-center mb-3 text-purple-300"><KeyRound className="h-7 w-7" /></div><h3 className="text-base font-black text-purple-300">New Room Code Generated!</h3><p className="mt-1 text-xs text-slate-300">Aapka room lock ho chuka hai. Naya Secret Code yeh hai:</p><div className="my-4 px-6 py-2 rounded-xl bg-black/80 border border-amber-400/60 text-amber-300 font-black text-2xl tracking-widest shadow-inner">{generatedPin}</div><button type="button" onClick={() => setGeneratedPin(null)} className="w-full py-3 rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold text-xs shadow-lg active:scale-95 transition-transform">Okay, Got It 👍</button></div></div>)}
      {showHostFreePopup && isHost && (<div className="absolute inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-md animate-fade-in px-4"><div className="relative flex flex-col items-center max-w-sm w-full rounded-3xl bg-slate-900 border border-amber-500/50 p-6 shadow-[0_0_35px_rgba(234,179,8,0.4)] text-center"><div className="absolute -top-10 animate-bounce"><div className="h-20 w-20 rounded-full bg-gradient-to-tr from-amber-400 to-purple-600 p-1 shadow-xl"><div className="h-full w-full rounded-full bg-black flex items-center justify-center overflow-hidden"><img src="/images/jalwa-1.gif" alt="Rocket GIF" className="h-14 w-14 object-contain" /></div></div></div><h3 className="mt-8 text-lg font-black text-amber-300">Room Popularity 100% Reached!</h3><p className="mt-2 text-xs text-slate-300">Host! Aapka Jalwa Rocket trigger ho chuka hai. Free (0 Coins) mein play karein!</p><div className="mt-6 flex gap-3 w-full"><button type="button" onClick={() => { setShowHostFreePopup(false); setShowRocketAnimation(true); setTimeout(() => setShowRocketAnimation(false), 5000); }} className="flex-1 py-3 rounded-xl bg-gradient-to-r from-amber-500 to-purple-600 text-white font-bold text-xs shadow-lg active:scale-95 transition-transform">Play Free (0 Coins) 🚀</button><button type="button" onClick={() => setShowHostFreePopup(false)} className="px-4 py-3 rounded-xl bg-white/10 text-slate-300 font-semibold text-xs active:scale-95 transition-transform">Close</button></div></div></div>)}
      {showRocketAnimation && (<div className="absolute inset-0 z-50 pointer-events-none flex items-center justify-center bg-black/80 backdrop-blur-md animate-fade-in"><div className="flex flex-col items-center"><div className="relative animate-bounce"><div className="absolute -inset-8 rounded-full bg-gradient-to-r from-amber-400 via-purple-500 to-cyan-400 opacity-90 blur-2xl animate-pulse" /><div className="relative grid h-52 w-52 place-items-center p-2"><img src="/images/jalwa-1.gif" alt="Jalwa Rocket GIF" className="h-full w-full object-contain drop-shadow-[0_0_25px_rgba(255,215,0,0.9)] animate-pulse" /></div></div><h2 className="mt-6 text-xl font-black tracking-wider text-amber-300 drop-shadow-[0_2px_6px_rgba(0,0,0,0.9)]">🚀 JALWA ROCKET LAUNCHED! 100% POPULAR! 🚀</h2></div></div>)}
      ...
      <div className="relative z-10 flex min-h-0 flex-1 flex-col overflow-hidden bg-transparent">
        ...
        <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden justify-between">
          <div className="relative flex-1 min-h-0 overflow-y-auto bg-transparent pt-1 px-1 no-scrollbar"><SeatGrid seats={room.seats.map(s => ({ ...s, className: s.index === glowingSeatIndex ? "ring-4 ring-amber-400 shadow-[0_0_25px_rgba(234,179,8,0.9)] scale-105 transition-all duration-300" : "" }))} seatCount={effectiveSeatCount} host={room.host} roomId={roomId} isHost={isHost} onSeatTap={openMemberProfile} onJoinSeat={joinSeat} onHostTap={onHostTap} /></div>
          {activeFlyingEmoji && (<div className="absolute inset-0 z-50 pointer-events-none flex items-center justify-center"><div className="animate-ping absolute h-16 w-16 rounded-full bg-amber-400 opacity-75" /><img src={activeFlyingEmoji.emojiUrl} alt="" className="h-16 w-16 object-contain animate-bounce drop-shadow-[0_0_15px_rgba(255,215,0,1)]" /></div>)}
          <div className="shrink-0 flex flex-col gap-1.5 px-3 pb-1">
            <section className="grid grid-cols-[minmax(0,1.7fr)_minmax(85px,.7fr)] gap-1.5">
              <div className="flex min-w-0 max-h-[190px] flex-col overflow-hidden rounded-2xl border border-white/20 bg-black/50 backdrop-blur-xl shadow-xl">
                <div className="flex h-6 shrink-0 items-center gap-1.5 border-b border-white/10 px-2 bg-amber-500/10 text-amber-200"><span className="text-[10px]">📢</span><p className="text-[9px] font-semibold truncate flex-1">{announcement || "Welcome to the room! Follow rules."}</p></div>
                <div ref={chatContainerRef} className="min-h-0 flex-1 overflow-y-auto px-2 py-1 space-y-1 no-scrollbar scroll-smooth">{visibleMessages.length === 0 ? (<div className="flex h-full items-center justify-center text-center text-white/40"><p className="text-[10px] font-medium">{announcement || "Welcome to the room!"}</p></div>) : (visibleMessages.map(m => { const avatar = m.user?.avatar || m.sender_avatar, fallback = (m.user?.username || m.sender_username || "U").trim().charAt(0).toUpperCase(); return (<div key={m.id} className="flex min-w-0 items-start gap-1 text-[10px]"><div className="grid h-3.5 w-3.5 shrink-0 overflow-hidden rounded-full bg-amber-500/20 text-[7px] font-bold text-amber-300 place-items-center border border-amber-500/30">{avatar ? <img src={avatar} alt="" className="h-full w-full object-cover" onError={e => { e.currentTarget.style.display = "none"; }} /> : fallback}</div><span className="min-w-0 break-words text-white/90">{m.text || m.message || ""}</span></div>); }))}</div>
                <div onClick={() => { if (!roomSettings.chat_enabled) return; setShowMiniInputPopup(true); }} className={cn("mx-1.5 mb-1.5 flex h-6 shrink-0 items-center gap-1.5 rounded-full border border-white/20 bg-white/10 px-3 shadow-inner cursor-pointer select-none", !roomSettings.chat_enabled && "opacity-50")}><span className="min-w-0 flex-1 text-[10px] font-medium text-white/40 truncate">{roomSettings.chat_enabled ? "Say something..." : "Chat disabled"}</span><Smile className="h-3 w-3 shrink-0 text-amber-300" /></div>
              </div>
              <div className="flex min-w-0 flex-col justify-between items-end gap-1 max-h-[190px]">
                <button type="button" onClick={tap(openPopularity)} style={{ height: "75px" }} className={cn(buttonClass, "relative group flex items-center justify-center bg-transparent border-0 p-0 w-full shrink-0 shadow-none")}><div className="relative h-[75px] w-[75px] flex items-center justify-center"><img src="/images/jalwa-1.gif" alt="Rocket GIF" className="h-[75px] w-[75px] object-contain animate-bounce drop-shadow-[0_0_10px_rgba(234,179,8,0.5)]" /></div></button>
                <div style={{ height: "105px" }} className="relative w-full shrink-0 overflow-hidden rounded-xl border border-white/20 bg-black/50 backdrop-blur-md flex items-center justify-center shadow-lg">{currentSlide && <img src={currentSlide.image_url} alt={currentSlide.title} className="h-full w-full object-cover rounded-xl" />}</div>
              </div>
            </section>
          </div>
        </div>
      ...
      {emojiOpen && (<div className="absolute inset-0 z-50 flex flex-col justify-end bg-black/60 backdrop-blur-sm animate-fade-in"><div className="absolute inset-0" onClick={() => setEmojiOpen(false)} /><div className="relative z-10 w-full rounded-t-3xl bg-slate-900/95 border-t border-amber-500/30 p-4 shadow-2xl backdrop-blur-xl animate-slide-up max-h-[60dvh] flex flex-col"><div className="mx-auto mb-3 h-1.5 w-12 rounded-full bg-slate-700" /><div className="flex items-center justify-between pb-3 border-b border-slate-800"><div className="flex items-center gap-2"><Smile className="h-4 w-4 text-amber-400" /><span className="text-xs font-bold tracking-wide text-amber-400 uppercase">Send Emoji to Seat</span></div><button type="button" onClick={() => setEmojiOpen(false)} className="text-xs font-semibold text-slate-400 hover:text-white">Close</button></div><div className="py-2.5 border-b border-slate-800/80"><p className="text-[10px] text-slate-400 mb-2 font-medium">Select Target Seat (Optional):</p><div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar"><button type="button" onClick={() => setSelectedTargetSeat(null)} className={cn("px-3 py-1.5 rounded-lg text-[10px] font-bold whitespace-nowrap transition-all border", selectedTargetSeat === null ? "bg-amber-500 text-black border-amber-400 shadow-md" : "bg-slate-800 text-slate-300 border-slate-700 hover:bg-slate-700")}>General Chat</button>{room.seats.map(seat => (<button key={seat.index} type="button" onClick={() => setSelectedTargetSeat(seat.index)} className={cn("flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[10px] font-semibold whitespace-nowrap transition-all border", selectedTargetSeat === seat.index ? "bg-amber-500 text-black border-amber-400 shadow-md" : "bg-slate-800 text-slate-300 border-slate-700 hover:bg-slate-700")}><UserCheck className="h-3 w-3" /><span>Seat {seat.index + 1} {seat.user?.username ? `(${seat.user.username})` : "(Empty)"}</span></button>))}</div></div><div className="flex-1 overflow-y-auto py-3"><ChatEmojiSheet open={emojiOpen} onClose={() => setEmojiOpen(false)} onPick={handlePickEmoji} /></div></div></div>)}
      {userMoreOpen && (<div className="fixed inset-0 z-[2147483000] flex items-end justify-center bg-black/60 p-0 backdrop-blur-sm animate-fade-in" onClick={() => setUserMoreOpen(false)}><div className="w-full max-w-sm rounded-t-3xl border-t border-x border-white/20 bg-[#100719]/95 p-4 text-white shadow-2xl backdrop-blur-md flex flex-col animate-slide-up" onClick={e => e.stopPropagation()}><div className="flex items-center justify-between pb-3 border-b border-white/10"><div className="flex items-center gap-2"><Info className="h-4 w-4 text-cyan-400" /><h3 className="text-xs font-black uppercase tracking-wider text-cyan-300">Room Options</h3></div></div><div className="py-4 space-y-2 text-xs"><button type="button" onClick={() => onToggleSpeaker()} className="w-full flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/10 active:scale-95 transition"><div className="flex items-center gap-3"><div className={`p-2 rounded-lg ${speakerMuted ? "bg-rose-500/20 text-rose-300" : "bg-cyan-500/20 text-cyan-300"}`}>{speakerMuted ? <VolumeX className="h-4 w-4" /> : <Volume2 className="h-4 w-4" />}</div><div className="text-left"><p className="font-bold text-white">Speaker Output</p><p className="text-[10px] text-white/50">{speakerMuted ? "Awaaz band hai (Muted)" : "Awaaz chalu hai"}</p></div></div><span className={`text-[10px] font-bold px-2.5 py-1 rounded-lg ${speakerMuted ? "bg-rose-500/30 text-rose-300" : "bg-cyan-500/30 text-cyan-300"}`}>{speakerMuted ? "Unmute" : "Mute"}</span></button><button type="button" onClick={() => { setUserMoreOpen(false); onShare(); }} className="w-full flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/10 active:scale-95 transition"><div className="flex items-center gap-3"><div className="p-2 rounded-lg bg-purple-500/20 text-purple-300"><Share2 className="h-4 w-4" /></div><div className="text-left"><p className="font-bold text-white">Share Room</p><p className="text-[10px] text-white/50">Doston ke sath room link share karein</p></div></div><span className="text-[10px] font-semibold text-purple-300 bg-purple-500/20 px-2.5 py-1 rounded-lg">Share</span></button><button type="button" onClick={() => { setUserMoreOpen(false); onReport(); }} className="w-full flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/10 active:scale-95 transition"><div className="flex items-center gap-3"><div className="p-2 rounded-lg bg-amber-500/20 text-amber-300"><Flag className="h-4 w-4" /></div><div className="text-left"><p className="font-bold text-white">Report Room</p><p className="text-[10px] text-white/50">Agar koi kharabi ya violation hai</p></div></div><span className="text-[10px] font-semibold text-amber-300 bg-amber-500/20 px-2.5 py-1 rounded-lg">Report</span></button></div><button type="button" onClick={() => { setUserMoreOpen(false); onExit(); }} className="w-full py-3 rounded-xl bg-gradient-to-r from-rose-600 to-red-600 text-white font-bold text-xs shadow-lg active:scale-95 transition flex items-center justify-center gap-2"><LogOut className="h-4 w-4" /><span>Leave Room</span></button></div></div>)}

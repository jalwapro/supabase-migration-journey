...
export function HostRoomSettings({ roomId, open, onClose, onSettingsChange, onOpenMusic, speakerMuted, onToggleSpeaker }: Props) {
  ...
  return (
    <>
        ...
        >
          <div className="mb-2.5 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-1.5">
              {activeView !== "main" ? (
                <button type="button" onClick={() => { setActiveView("main"); setManagementMode(null); }} className="grid h-7 w-7 place-items-center rounded-full bg-white/10 active:scale-95 transition" aria-label="Back">
            ...
            {activeView === "main" ? (
              <>
                <div className="flex items-center justify-between rounded-xl border border-white/10 bg-white/5 px-2.5 py-1.5">
                  ...
                  <button type="button" onClick={onToggleSpeaker} className={`px-3 py-1 rounded-lg text-[10px] font-bold ${speakerMuted ? "bg-rose-500/20 text-rose-300 border border-rose-500/40" : "bg-cyan-500/20 text-cyan-300 border border-cyan-500/40"}`}>
                ...
                <div className="flex items-center justify-between rounded-xl border border-white/10 bg-white/5 px-2.5 py-1.5">
                  ...
                  <select value={settings.seat_count ?? 8} disabled={saving === "seat_count"} onChange={e => void update("seat_count", Number(e.target.value))} className="bg-black/60 border border-white/20 rounded-lg px-2 py-0.5 text-[10px] font-semibold text-white outline-none">
      ...
      <HostRoomManagementSheet
        roomId={roomId}
        open={managementMode !== null}
        mode={managementMode ?? "kick"}
...
function ActionButton({ icon, label, onClick, busy }: { icon: React.ReactNode; label: string; onClick: () => void; busy?: boolean }) {
  return (
    <button type="button" disabled={busy} onClick={onClick} className="group flex flex-col items-center justify-center gap-1 p-1.5 rounded-xl bg-gradient-to-b from-white/15 to-white/5 border border-white/20 shadow-[0_3px_8px_rgba(0,0,0,0.4),inset_0_1px_1px_rgba(255,255,255,0.2)] active:translate-y-0.5 transition-all disabled:opacity-50">
...
function ToggleMini({ icon, label, value, busy, onToggle }: { icon: React.ReactNode; label: string; value: boolean; busy: boolean; onToggle: (val: boolean) => void }) {
  return (
    <button type="button" disabled={busy} onClick={() => onToggle(!value)} className={`flex items-center justify-between p-1.5 rounded-lg border transition-all ${value ? "bg-amber-500/30 border-amber-400 shadow-[0_0_8px_rgba(245,158,11,0.3)]" : "bg-white/5 border-white/10"} disabled:opacity-50`}>

    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="rounded-t-3xl border-white/10 bg-[#0b0710] p-0 text-white" data-adaptive="neon">
        ...
        <div className="flex flex-col gap-1.5 p-3">
          {!user && (
            <SheetAction icon={<UserPlus className="h-4 w-4" />} label="Join this seat" onClick={onJoin} />
          )}

          {user && isMe && (
            <>
              <SheetAction
                icon={user.mic === "off" || user.mic === "muted" ? <Mic className="h-4 w-4" /> : <MicOff className="h-4 w-4" />}
                label={user.mic === "off" || user.mic === "muted" ? "Turn mic on" : "Turn mic off"}
                onClick={onToggleMic}
              />
            ...
            <>
              <div className="flex items-center gap-3 rounded-xl bg-white/[0.03] p-2.5">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="flex items-center gap-1 text-[11px] text-pink-300">
                    <Heart className="h-3 w-3 fill-current" /> {formatCount(user.popularity)}
                  </p>
                </div>
              </div>
...
function SheetAction({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-2.5 rounded-xl px-3 py-2.5 text-left text-sm font-medium transition-colors hover:bg-white/[0.06] ${
        destructive ? "text-red-400" : "text-white/85"
      }`}
    >
      {icon}
      {label}

  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const row = (Array.isArray(stats) ? stats[0] : stats) as Partial<PopularityData> | null;
        if (!row) {
          setData(emptyData(resolvedHostId));
          return;
        }
        ...
        setData({
          ...
          weekly_followers: Number(row.weekly_followers) || 0,
        });
      } catch (err) {
        if (!active) return;
        setData(emptyData());

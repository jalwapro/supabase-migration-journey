...
function LayerRow({ layer, selectedId, onSelect, onToggle, onReorder, onRename }: { layer: StudioLayer; selectedId: string | null; onSelect: (id: string) => void; onToggle: (id: string, patch: Partial<Pick<StudioLayer, "visible" | "locked" | "expanded">>) => void; onReorder: (id: string, direction: "up" | "down") => void; onRename: (id: string, name: string) => void; }) {
  ...
  const [name, setName] = useState(layer?.name ?? "Layer");
  const children = Array.isArray(layer?.children) ? layer.children.filter(Boolean) : [];
  const hasChildren = children.length > 0;
  const commitName = () => { if (!layer?.id) return; onRename(layer.id, name); setEditing(false); };
  if (!layer || typeof layer !== "object" || !layer.id) return null;
  return <div>
    <div className={`group flex items-center gap-1 rounded-md px-1 py-1 ${selectedId === layer.id ? "bg-primary/10 ring-1 ring-primary/20" : "hover:bg-muted"}`} style={{ paddingLeft: 4 + (layer.depth ?? 0) * 14 }}>
    ...
    {layer.expanded && hasChildren && <div>{children.map(child => <LayerRow key={child.id} layer={child} selectedId={selectedId} onSelect={onSelect} onToggle={onToggle} onReorder={onReorder} onRename={onRename} />)}</div>}
  </div>;
}
...
export function ExistingAppDetectionPanel({ route }: { route?: string | null }) {
  ...
  const screens = useMemo(() => { void revision; try { return detectExistingApp(); } catch { return []; } }, [revision]);
  const screen = findDetectedScreen(screens, route || "/");
  const [layers, setLayers] = useState<StudioLayer[]>([]);
  useEffect(() => { try { setLayers(screen ? buildLayerTree(screen) : []); } catch { setLayers([]); } setSelectedId(null); }, [screen?.id, revision]);

  const components = (screen?.components || []).filter(Boolean).filter((item) => `${item.name} ${item.type} ${item.source || ""} ${item.action?.target || ""}`.toLowerCase().includes(query.toLowerCase()));
  ...
  return <div className="rounded-xl border bg-background p-3">
    ...
    <div className="mb-3 rounded-lg border bg-muted/20 p-2"><div className="mb-2 flex items-center gap-1.5"><Layers3 className="h-3.5 w-3.5 text-primary" /><span className="text-[10px] font-bold uppercase tracking-widest">Layers</span><span className="ml-auto text-[9px] text-muted-foreground">{layers.length} root</span></div><div className="max-h-72 overflow-auto">{layers.filter(Boolean).map(layer => <LayerRow key={layer.id} layer={layer} selectedId={selectedId} onSelect={setSelectedId} onToggle={toggleLayer} onReorder={moveLayer} onRename={rename} />)}{!layers.length && <p className="py-4 text-center text-[10px] text-muted-foreground">No detected layers for this route.</p>}</div>{selectedId && <p className="mt-2 truncate border-t pt-2 text-[9px] text-muted-foreground">Selected: {selectedId}</p>}</div>

...
export function RoomStudioElementPreview({ element, roomType: _roomType, interactive: _interactive = false }: { element: LayoutElement; roomType: RoomType; interactive?: boolean }) {
  const common = 'w-full h-full overflow-hidden';
  switch (element.type) {
    case 'room-header': return <div className={`${common} flex items-center justify-between px-3 bg-black/35 backdrop-blur border-b border-white/10`}><div className="flex items-center gap-2"><div className="w-9 h-9 rounded-full bg-gradient-to-br from-purple-400 to-pink-500"/><div><div className="text-white text-xs font-semibold">Live Room</div><div className="text-white/40 text-[9px]">12.4K online</div></div></div><div className="flex gap-1"><span className="px-2 py-1 rounded-full bg-white/10 text-white/70 text-[9px]">Follow</span><span className="w-7 h-7 rounded-full bg-white/10 flex items-center justify-center text-white">•••</span></div></div>;
    case 'room-announcement': case 'room-title': return <div className={`${common} flex items-center px-3 bg-white/5`}><span className="text-[10px] text-white/70">🔥 Welcome to the room • Enjoy the live session</span></div>;
    case 'host-avatar': return <div className={`${common} flex flex-col items-center justify-center`}><div className="w-16 h-16 rounded-full bg-gradient-to-br from-purple-400 to-pink-500 ring-2 ring-white/20"/><span className="text-white text-[10px] mt-1">Host</span></div>;
    case 'seat': return <div className={`${common} rounded-xl bg-white/[0.06] border border-white/10 flex flex-col items-center justify-center relative`}><span className="absolute top-1 left-1 text-[8px] text-white/35">Seat {String(element.data?.seatNumber ?? '').padStart(2, '0')}</span><div className="w-11 h-11 rounded-full bg-gradient-to-br from-slate-400 to-slate-700 ring-2 ring-white/10"/><span className="text-[9px] text-white/60 mt-1">Available</span><span className="text-[8px] text-white/30">🎙</span></div>;
    case 'seat-avatar': return <div className={`${common} flex items-center justify-center`}><div className="w-12 h-12 rounded-full bg-gradient-to-br from-cyan-400 to-blue-600"/></div>;
    case 'seat-lock': return <div className={`${common} flex items-center justify-center text-white/70`}>🔒</div>;
    case 'mic-icon': return <div className={`${common} flex items-center justify-center text-white`}>🎙</div>;
    case 'chat-panel': return <div className={`${common} rounded-xl bg-black/35 backdrop-blur border border-white/10 p-2`}><div className="text-white/50 text-[8px] mb-1">Live chat</div><div className="space-y-1"><div className="text-[8px] text-white/75"><b className="text-pink-300">Ahsan</b> Welcome everyone 👋</div><div className="text-[8px] text-white/65"><b className="text-cyan-300">Sara</b> Nice room!</div><div className="text-[8px] text-white/60"><b className="text-purple-300">Ali</b> ❤️❤️❤️</div></div></div>;
    case 'chat-message': return <div className={`${common} flex items-center px-2 text-[9px] text-white/70`}>Ahsan: Welcome 👋</div>;
    case 'gift-button': case 'send-gift-button': return <div className={`${common} rounded-full bg-gradient-to-r from-pink-500 to-purple-600 flex items-center justify-center text-white font-semibold text-[10px]`}>🎁 Gift</div>;
    case 'gift-panel': return <div className={`${common} rounded-xl bg-black/40 border border-white/10 p-2 flex gap-1 items-end justify-center`}><span>🌹</span><span>💎</span><span>🚀</span><span>👑</span></div>;
    case 'coin-balance': return <div className={`${common} flex items-center justify-center rounded-full bg-black/30 text-yellow-300 text-[9px]`}>🪙 12,450</div>;
    case 'follow-button': return <div className={`${common} rounded-full bg-white/10 flex items-center justify-center text-white text-[9px]`}>+ Follow</div>;
    case 'share-button': return <div className={`${common} flex items-center justify-center text-white text-sm`}>↗</div>;
    case 'more-button': return <div className={`${common} flex items-center justify-center text-white text-sm`}>•••</div>;
    case 'settings-button': return <div className={`${common} flex items-center justify-center text-white text-sm`}>⚙</div>;
    case 'bottom-toolbar': return <div className={`${common} flex items-center justify-around bg-black/45 border-t border-white/10`}><span>🎙</span><span>🎮</span><span>🎁</span><span>✨</span><span>⚙</span></div>;
    ...
    case 'custom-text': return <div className={`${common} flex items-center px-2`} style={{ color: element.style?.color || '#fff', fontSize: element.style?.fontSize || 14, fontWeight: element.style?.fontWeight || '400', textAlign: element.style?.textAlign || 'left' }}>{String(element.data?.text ?? 'Your text')}</div>;
    case 'custom-image': return element.data?.src ? <img src={String(element.data.src)} alt="Custom" className="w-full h-full object-cover" /> : <div className={`${common} flex items-center justify-center bg-white/5 text-white/40 text-[9px]`}>🖼 Custom Image</div>;

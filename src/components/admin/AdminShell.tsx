...
export const ADMIN_NAV: NavGroup[] = [
  ...
  { label: "Content", items: [
    ...
    { to: "/admin/frames", label: "Profile Frames", icon: ImagePlus },
  ...
  { label: "App Customization", items: [
    { to: "/admin/app-customization", label: "Customization Studio", icon: SlidersHorizontal },
    { to: "/admin/room-layouts", label: "Room Layout Studio", icon: Layout },
  ] },
  { label: "Reports", items: [
...
function MobileNav({ pathname }: { pathname: string }) {
  return <div className="sticky top-0 z-30 flex items-center gap-2 overflow-x-auto border-b border-border bg-background/95 px-3 py-2 backdrop-blur md:hidden">{ADMIN_NAV.flatMap((g) => g.items).map((it, index) => { const active = it.to === "/admin" ? pathname === "/admin" : pathname.startsWith(it.to); return <Link key={`${it.to}-${index}`} to={it.to} className={`flex shrink-0 items-center gap-1.5 rounded-full px-3 py-1.5 text-xs ${active ? "bg-primary text-primary-foreground" : "bg-card/60 text-muted-foreground"}`}><it.icon className="h-3.5 w-3.5" />{it.label}</Link>; })}</div>;
}

export function AdminPageHeader({ title, subtitle, right }: { title: string; subtitle?: string; right?: ReactNode }) {
  return <div className="mb-5 flex flex-wrap items-end justify-between gap-3"><div><h1 className="text-xl font-bold md:text-2xl">{title}</h1>{subtitle && <p className="mt-0.5 text-xs text-muted-foreground md:text-sm">{subtitle}</p>}</div>{right}</div>;
}

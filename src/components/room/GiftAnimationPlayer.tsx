...
/** Live room gift renderer plus the realtime entrance renderer. */
export function GiftAnimationPlayer({ roomId }: { roomId: string }) {
  ...
  useEffect(() => {
    ...
    if (audioSrc) audioTimer = window.setTimeout(() => { audio = new Audio(audioSrc); audio.volume = 1; void audio.play().catch(() => undefined); }, delayMs);
  ...
  return (
    <>
      ...
      <div className="pointer-events-none fixed inset-0 overflow-hidden" style={{ zIndex: 2147483647 }}>
        <div style={style}>
          {isVideo && src ? <GiftGLVideo key={`${active.id}:${cfg.delayMs}:${cfg.endMs ?? "clip"}`} src={src} config={cfg} muted loop={Boolean(active.gift_loop || cfg.loop)} objectFit={fit} className="h-full w-full" style={mergedMediaStyle} onError={() => setActive(null)} /> : image ? <img src={image} alt={active.gift_name ?? "Gift"} className="h-full w-full" style={mergedMediaStyle} onError={() => setActive(null)} /> : <div className="grid h-full w-full place-items-center text-[clamp(64px,22vw,180px)]">{active.gift_emoji || "🎁"}</div>}
        </div>
        <div className="absolute left-4 top-16 rounded-full border border-white/20 bg-black/55 px-3 py-1.5 text-xs font-semibold text-white">{active.sender_username || "User"} sent {active.gift_name || "Gift"}{numberValue(active.quantity, 1) > 1 ? ` ×${active.quantity}` : ""}</div>
      </div>
    </>
  );
}

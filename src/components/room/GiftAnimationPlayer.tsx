...
/** Crash-safe, self-contained gift renderer. It avoids the old GiftRender/GiftMedia/SVGA dependency chain that produced the production TDZ error. */
export function GiftAnimationPlayer({ roomId }: { roomId: string }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "gift_sends", filter: `room_id=eq.${roomId}` }, payload => {
        if (!alive) return;

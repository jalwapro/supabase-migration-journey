...
export function GiftAnimationPlayer({ roomId }: { roomId: string }) {
  ...
  const enqueueGift = useCallback((row: GiftRow) => {
    if (!row?.id) return;
    // The sender receives an optimistic local event and then the same INSERT from Realtime.
    // Deduplicate those events and queue rapid multi-gift sends instead of replacing playback.
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`gift-player-${roomId}`).on("postgres_changes", { event: "INSERT", schema: "public", table: "gift_sends", filter: `room_id=eq.${roomId}` }, payload => {
      if (!alive) return;
      enqueueGift(payload.new as GiftRow);
    }).subscribe();

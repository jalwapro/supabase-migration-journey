      ...
      <div className="relative flex flex-col items-center" style={{ animation: "spotlightPop 0.6s cubic-bezier(0.34,1.56,0.64,1)" }}>
        <div className="relative">
          ...
          <div className="relative h-40 w-40 overflow-hidden rounded-full border-4 border-amber-300 shadow-[0_0_80px_20px_rgba(255,215,0,0.6)]">
            {avatar ? <img src={avatar} alt={username} className="h-full w-full object-cover" /> : <div className="grid h-full w-full place-items-center bg-gradient-to-br from-fuchsia-500 to-purple-800 text-5xl font-black text-white">{username.slice(0, 1).toUpperCase()}</div>}
          </div>
        </div>
        <div className="mt-4 rounded-full bg-gradient-to-r from-amber-400 via-yellow-500 to-amber-400 px-6 py-2 text-lg font-black text-black shadow-2xl">{label}</div>

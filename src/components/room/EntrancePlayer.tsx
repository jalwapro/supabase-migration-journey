...
export function EntrancePlayer({ event, onDone }: { event: RoomEntranceEvent; onDone: () => void }) {
  ...
  const baseDuration = Math.min(Math.max(event.duration_ms ?? 2400, 1800), 12000);
  ...
  useEffect(() => {
    ...
    const playback = reduce ? 1100 : configuredEnd;
    const total = Math.max(300, delay + playback + Math.max(0, config.holdMs));
    const doneTimer = window.setTimeout(finish, total);

    // Full-volume fallback audio is used only for non-video media.
  ...
  }, [event.id, event.sound_url, hasVideo, isDirectVideo, config.delayMs, config.holdMs, configuredEnd]);
  ...
  return (
    <>
      {hasVideo && event.media_url ? (
        <div className={`pointer-events-none fixed inset-x-0 top-[58px] bottom-[62px] z-[998] flex justify-center overflow-hidden ${visible ? "opacity-100" : "opacity-0"}`} style={{ visibility: visible ? "visible" : "hidden" }} aria-hidden>

...
type Props = {
  open: boolean; onClose: () => void; seatCount: number; maxSeats?: number; onSeats: (next: number) => void | Promise<void>; onMusic: () => void; onSettings: () => void; onInvite: () => void; onRanking: () => void;
...
type GiftRow = { id: string; sender_username: string | null; receiver_username: string | null; gift_name: string | null; gift_emoji: string | null; coins_spent: number | null; created_at: string };

const normalizeSeats = (v: number, max = MAX_SEATS) => Number.isFinite(Number(v)) ? Math.min(max, Math.max(MIN_SEATS, Math.round(Number(v)))) : MIN_SEATS;
const roomIdFromUrl = () => typeof window === "undefined" ? null : window.location.pathname.match(/\/room\/([^/?#]+)/)?.[1] ?? null;

export function HostRoomMoreSheet({ open, onClose, seatCount, maxSeats = MAX_SEATS, onSeats, onMusic, onSettings, onInvite, onRanking, onMembers, onAnnouncement, onShare, onModerators, onReports, onGiftActivity, onStats, onEndRoom, onSeatManagement }: Props) {
  const selected = normalizeSeats(seatCount, maxSeats);
  ...
  const loadGifts = async () => {
    if (!roomId) return;
    setLoading(true);
    const { data, error } = await supabase.from("gift_sends").select("id,sender_username,receiver_username,gift_name,gift_emoji,coins_spent,created_at").eq("room_id", roomId).order("created_at", { ascending: false }).limit(50);
    if (error) { toast.error(error.message); setLoading(false); return; }
  ...
  const stats = useMemo(() => {
    const total = gifts.reduce((sum, g) => sum + Number(g.coins_spent ?? 0), 0);
    const senders = new Set(gifts.map(g => g.sender_username).filter(Boolean)).size;
    return { total, senders, gifts: gifts.length };
  }, [gifts]);

  if (!open) return null;
  const change = (d: number) => { const n = normalizeSeats(selected + d, maxSeats); if (n !== selected) void onSeats(n); };

...
export function HostRoomMoreSheet({ open, onClose, seatCount, onSeats, onMusic, onSettings, onInvite, onRanking }: Props) {
  if (!open) return null;
  ...
  return (<>
    ...
    <section className="fixed bottom-0 left-1/2 z-[91] w-full max-w-[480px] -translate-x-1/2 rounded-t-3xl border-t-2 border-[color:var(--secondary)] bg-[color:var(--primary)] p-5 pb-[calc(20px+env(safe-area-inset-bottom))] text-white shadow-2xl">
      ...
      <div className="mb-4 flex items-center justify-between"><div><h2 className="text-lg font-extrabold">Host Controls</h2><p className="text-[11px] text-white/80">Manage your live voice room</p></div><button type="button" onClick={onClose} className="grid h-9 w-9 place-items-center rounded-full border border-white/50 bg-white/15" aria-label="Close host controls"><X className="h-4 w-4" /></button></div>
      <div className={`${card} mb-3 p-3`}><div className="mb-2 flex items-center justify-between"><div className="flex items-center gap-2"><LayoutGrid className="h-4 w-4 text-[color:var(--secondary)]" /><span className="text-sm font-bold">Room Seats</span></div><span className="text-xs text-white/80">{selected} total</span></div>
        ...
        <div className="mt-2 flex items-center justify-center gap-2 text-[10px] text-white/70"><span>Minimum: {MIN_SEATS}</span><span>•</span><span>Maximum: {MAX_SEATS}</span><span>•</span><span>Host uses seat 1</span></div><p className="mt-2 text-[10px] text-white/70">Host apni marzi se 4 se 20 tak koi bhi seat count set kar sakta hai.</p>
      </div>
      <div className="grid grid-cols-2 gap-2">
        ...
        <button type="button" onClick={onClose} className={`${action} col-span-2`}><LayoutGrid className="h-5 w-5 text-[color:var(--secondary)]" /><span><b className="block text-xs">Seat Management</b><small className="text-[10px] text-white/75">Open, lock, unlock, invite or remove users from seats</small></span></button>
      </div>

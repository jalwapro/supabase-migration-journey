...
const SEAT_LAYOUTS = [4, 8, 12, 16, 20] as const;
type SeatCapacity = (typeof SEAT_LAYOUTS)[number];

const normalizeSeats = (value: number): SeatCapacity => {
  const numeric = Number(value);
  if (!Number.isFinite(numeric)) return 4;
  return SEAT_LAYOUTS.reduce((closest, candidate) =>
...
export function HostRoomMoreSheet({
  ...
  const selectedIndex = SEAT_LAYOUTS.indexOf(selected);
  const changeSeats = (direction: -1 | 1) => {
    const nextIndex = Math.min(SEAT_LAYOUTS.length - 1, Math.max(0, selectedIndex + direction));
    const next = SEAT_LAYOUTS[nextIndex];
    ...
    <>
      ...
      <section className="fixed bottom-0 left-1/2 z-[91] w-full max-w-[480px] -translate-x-1/2 rounded-t-3xl border-t-2 border-[color:var(--secondary)] bg-[color:var(--primary)] p-5 pb-[calc(20px+env(safe-area-inset-bottom))] text-white shadow-2xl">
        ...
        <div className={`${card} mb-3 p-3`}>
          ...
          <div className="flex items-center justify-between gap-3"><button type="button" aria-label="Previous room seat layout" disabled={selectedIndex === 0} onClick={() => changeSeats(-1)} className="grid h-11 w-11 place-items-center rounded-xl border border-white/45 bg-white/15 disabled:opacity-30"><Minus className="h-5 w-5" /></button><div className="text-2xl font-black tabular-nums">{selected}</div><button type="button" aria-label="Next room seat layout" disabled={selectedIndex === SEAT_LAYOUTS.length - 1} onClick={() => changeSeats(1)} className="grid h-11 w-11 place-items-center rounded-xl border border-white/45 bg-white/15 disabled:opacity-30"><Plus className="h-5 w-5" /></button></div>
          <div className="mt-2 grid grid-cols-5 gap-1">{SEAT_LAYOUTS.map((layout) => <span key={layout} className={`text-center text-[9px] ${layout === selected ? "font-bold text-white" : "text-white/65"}`}>{layout}</span>)}</div>
          <p className="mt-2 text-[10px] text-white/70">Supported layouts: 4, 8, 12, 16 or 20 seats. Seat 1 is always the host.</p>
        </div>

...
export function useLiveKitRoom({ channel, uid, publish, video, enabled }: UseLiveKitRoomArgs) {
  ...
  useEffect(() => {
    ...
    const onSub = (track: RemoteTrack, _pub: RemoteTrackPublication, p: RemoteParticipant) => { refresh(p); if (track.kind === Track.Kind.Audio && !speakerMuted) { track.attach().forEach((el) => { const a = el as HTMLAudioElement; a.autoplay = true; document.body.appendChild(a); void a.play().catch(() => undefined); }); } };

...
/** Reliable room entrance stream: durable Postgres Changes + immediate local RPC + Broadcast. */
export function useRoomEntrances(roomId: string | null | undefined, localUserId?: string | null) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`room-entrances-${roomId}`, { config: { broadcast: { self: true } } })
      .on(
        "postgres_changes",
      ...
      .subscribe(async status => {
        if (status !== "SUBSCRIBED" || entranceFired || cancelled) return;
        entranceFired = true;

        // Do not wait for Postgres INSERT realtime to reach the joining browser.
        // The membership trigger may have inserted the row before this channel
        // subscribed. fire_room_entrance is debounced and returns that existing
        // row, or creates the fallback row for the authenticated user.
        ...
        // Always enqueue the RPC result locally. This is what guarantees that
        // the user sees their OWN entrance, even when the INSERT happened before
        // realtime subscription was established.
        enqueue(row);

        // Also fan it out immediately to every other browser in the room.

...
type ActiveVoiceRoom = { roomId: string; roomName?: string | null; roomAvatar?: string | null; roomRoute: string; userId: string; userRole?: string | null; isMinimized: boolean; connectionState?: string | null; microphoneMuted?: boolean; };
...
export function VoiceRoomSessionProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => { if (!user?.id) return; const onClick = (event: MouseEvent) => { const target = event.target as HTMLElement | null; const button = target?.closest('button') as HTMLButtonElement | null; if (!button) return; const label = [button.getAttribute('aria-label'), button.getAttribute('title'), button.textContent].filter(Boolean).join(' ').replace(/\s+/g, ' ').trim(); if (!/\bminimi[sz]e\b/i.test(label) && !/go\s*home.*room/i.test(label)) return; const match = window.location.pathname.match(/^\/room\/([^/?#]+)/); if (!match) return; const roomId = decodeURIComponent(match[1]); minimizeRoom({ roomId, roomRoute: `/room/${encodeURIComponent(roomId)}`, userId: user.id, userRole: 'user', connectionState: 'connected', microphoneMuted: false }); }; document.addEventListener('click', onClick, true); return () => document.removeEventListener('click', onClick, true); }, [user?.id, minimizeRoom]);
